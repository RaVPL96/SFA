...
GROUP BY audit_user ORDER by samples desc');
    ...
    function getCompetitors(){
        $this->db->select('`tbl_trans_survey_competitors`.`name` AS comp_name');
        $this->db->from('tbl_trans_survey_competitors');
        $query = $this->db->get();
        $result = $query->result_array();
        return $result;
    }
    function getSurveyDetails($dateFrom=null, $dateTo=null, $areaID = null, $territoryID = null, $rangeID = null,$survey_id=17) {
        ...
        $q1.=' case ';
//         
            $q1 .='when tbl_trans_survey_detail.competitor=\''.$a['comp_name'].'\' then `tbl_trans_survey_detail`.`question_1`';
            
        $q1.=' end AS '.$a['comp_name'].'_mrp,';
        
        $q1.=' case ';
//         
            $q1 .='when tbl_trans_survey_detail.competitor=\''.$a['comp_name'].'\' then `tbl_trans_survey_detail`.`question_2`';
            
        $q1.=' end AS '.$a['comp_name'].'_retail_price,';
        
        $q1.=' case ';
//         
            $q1 .='when tbl_trans_survey_detail.competitor=\''.$a['comp_name'].'\' then `tbl_trans_survey_detail`.`question_3`';
            
        $q1.=' end AS '.$a['comp_name'].'_wholesale_price,';
        
        $q1.=' case ';
//         
            $q1 .='when tbl_trans_survey_detail.competitor=\''.$a['comp_name'].'\' then `tbl_trans_survey_detail`.`question_4`';
            
        $q1.=' end AS '.$a['comp_name'].'_free_issue,';
        
        $q1.=' case ';
//         
            $q1 .='when tbl_trans_survey_detail.competitor=\''.$a['comp_name'].'\' then `tbl_trans_survey_detail`.`question_5`';
            
        $q1.=' end AS '.$a['comp_name'].'_discount,';
        
        $q1.=' case ';
//         
            $q1 .='when tbl_trans_survey_detail.competitor=\''.$a['comp_name'].'\' then `tbl_trans_survey_detail`.`question_6`';
