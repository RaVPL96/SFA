...
ORDER BY tbl_mst_region.id,tbl_mst_area.area_name,agency_mst.ag_name,invh.ag_cd,cd');
    ...
    function getTargetNewDemarcation($tyear, $tmonth, $area_cd = NULL) {
        $this->db->select('`tbl_mst_area`.`id` AS `area_cd`, `target_value`.`ag_cd`, `t_year`, `t_mon`, `tbl_mst_area`.`area_name`, `ag_name`, SUM(IF(tbl_mst_geography.range_id=1,`c_target`,0)) AS `c_target`, SUM(IF(tbl_mst_geography.range_id=2 OR tbl_mst_geography.range_id=5,`d_target`,0)) AS `d_target`, `a_target`, SUM(IF(tbl_mst_geography.range_id=4,`b_target`,0)) AS `b_target`, SUM(IF(tbl_mst_geography.range_id=3,`s_target`,0)) AS `s_target`, SUM(IF(tbl_mst_geography.range_id=5,`t_target`,0)) AS `t_target`, SUM(IF(tbl_mst_geography.range_id=6,`r_target`,0)) AS  `r_target`, SUM(IF(tbl_mst_geography.range_id=1,`acs_pc_target`,0)) AS `acs_pc_target`, SUM(IF(tbl_mst_geography.range_id=2,`bd_pc_target`,0)) AS `bd_pc_target`, `wd`, `p_wd`, `auto`');
        $this->db->from('`target_value`');
...
WHERE invd.a_Qty<>0 AND tbl_mst_item_category_purpose.id=3 AND tbl_mst_item_category.id=' . $category . ' AND invh.date_actual>=\'' . date('Y-m-01', strtotime($datepickermonth . ' -1 month')) . '\' AND invh.date_actual<=\'' . date('Y-m-t', strtotime($datepickermonth)) . '\' AND invh.b_a=\'A\' AND invh.tot_a_val<>0 ' . $str . '
...
WHERE invh.date_actual>=\'' . date('Y-m-01', strtotime($datepickermonth . ' -1 month')) . '\' AND invh.date_actual<=\'' . date('Y-m-t', strtotime($datepickermonth)) . '\' AND invh.b_a=\'A\' AND invh.tot_a_val<>0 ' . $str . '
GROUP BY invh.ag_cd,invh.date_actual ) a GROUP BY a.ag_cd';
        $result = $this->db->query($query)->result_array();
        //echo $this->db->last_query();
        return $result;
