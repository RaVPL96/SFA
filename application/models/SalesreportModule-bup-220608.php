...
class SalesreportModule extends CI_Model {
    ...
    /*----------------sandun-------------------*/
    /*----------------Item Wise Report-------------------*/
    function getAreaSumItemWise($areaID = null, $categoryID = null, $billingType = null, $dateFrom = null, $dateTo = null, $reportType = 'summery') {
        if ($reportType == 'summery') {
            if ($billingType == 'A') {
                $this->db->select('`invd`.`item`, `item_mst`.`des`, `item_mst`.`cat`, sum(`invd`.`a_Qty`), sum(`invd`.`a_Qty`*`invd`.`up`)');                
            }else{
                $this->db->select('`invd`.`item`, `item_mst`.`des`, `item_mst`.`cat`, sum(`invd`.`a_Qty`), sum(`invd`.`a_Qty`*`invd`.`up`)');
            }
        }elseif ($reportType == 'summery_territory') {
