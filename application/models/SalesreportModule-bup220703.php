...
class SalesreportModule extends CI_Model {
    ...
    /////////////////////////////
    /*sandun*/
    function getItemPcTotals($areaID = null, $territoryID = null, $categoryID = null, $routeID = null, $rangeID = null, $billingType = null, $dateFrom = null, $dateTo = null, $reportType = 'summery',$channel_id=1) {
        if ($reportType == 'summery_area') {//area wise sales summery data
            if ($billingType == 'A') {
                $this->db->select('invd.item, item_mst.des, item_mst.cat, sum(a_Qty), sum(invd.a_Qty*invd.up)');
            } else {
                $this->db->select('invd.item, item_mst.des, item_mst.cat, sum(a_Qty), sum(invd.a_Qty*invd.up)');
            }
        } elseif ($reportType == 'summery') {
        ...
        if ($reportType == 'summery_area') {//for area wise summery group them by area id
            if ($billingType == 'A') {
                $this->db->order_by('a.ag_cd, h.invno');
            } else {
                $this->db->order_by('a.ag_cd, h.invno');
            }
        } elseif ($reportType == 'summery') {
