...
';
    ...
    function getCancellation($dateFrom, $dateTo, $agencyCode = 'a', $reportType = 'summery', $areaId, $rangeID = '-1') {
        if ($reportType == 'summery') {
        ...
        } elseif ($reportType == 'territory_summery') {
            ...
            $this->db->select('`invh`.`invno`,`invh`.`date_book`,`invh`.`date_actual`,`invh`.`tot_b_val`,`invh`.`tot_c_val`,`invh`.`tot_a_val`,`tbl_mst_outlet`.`name`');
        }
