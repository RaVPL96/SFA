...
class HrModule extends CI_Model {
        ...
        for ($i = $begin; $i <= $end; $i->modify('+1 day')) {
            ...
            foreach ($result as $r) {
                ...
                if ($r['user_name'] != 'testc') {
                    ...
                    if($totMadeCalls==0 && $totPCalls==0 && $totUPCalls==0){//new version for made call support not installed may be
                        $totPCalls= $totMadeCalls = $totBills;
                    }
    ...
    function getMadeCallSummery($dateFrom = null, $dateTo = null, $user){
        /*
        echo 'SELECT T.user_id, T.visit_date, COUNT(T.made_calls) AS made_call_count,COUNT(T.productive_outlets) AS PC,COUNT(T.unproductive_outlets) AS UPC  FROM (SELECT m.user_id, m.visit_date, m.outlet_id AS made_calls,p.productive_outlets,u.unproductive_outlets,m.app_inv_no
FROM `tbl_trans_visit_log` m 
LEFT JOIN (SELECT user_id, visit_date, outlet_id AS productive_outlets FROM `tbl_trans_visit_log`  WHERE  is_productive=1 AND visit_date>=\''.$dateFrom.'\' AND visit_date<=\''.$dateTo.'\' AND user_id=\''.$user.'\' GROUP BY user_id, visit_date, outlet_id) p ON m.outlet_id=p.productive_outlets AND m.visit_date=p.visit_date AND m.user_id=p.user_id
LEFT JOIN (SELECT user_id, visit_date, outlet_id AS unproductive_outlets FROM `tbl_trans_visit_log` WHERE  is_productive=0 AND visit_date>=\''.$dateFrom.'\' AND visit_date<=\''.$dateTo.'\' AND user_id=\''.$user.'\' AND (user_id, outlet_id ,visit_date) NOT IN (SELECT user_id, outlet_id, visit_date FROM `tbl_trans_visit_log` WHERE is_productive=1 AND visit_date>=\''.$dateFrom.'\' AND visit_date<=\''.$dateTo.'\' AND user_id=\''.$user.'\' GROUP BY user_id, visit_date, outlet_id) GROUP BY user_id, visit_date, outlet_id) u ON m.outlet_id=u.unproductive_outlets AND m.visit_date=u.visit_date AND m.user_id=u.user_id GROUP BY m.user_id, m.visit_date, m.outlet_id) T WHERE T.visit_date>=\''.$dateFrom.'\' AND T.visit_date<=\''.$dateTo.'\' AND T.user_id=\''.$user.'\' GROUP BY T.user_id, T.visit_date';
        die();
...
LEFT JOIN (SELECT user_id, visit_date, outlet_id AS unproductive_outlets FROM `tbl_trans_visit_log` WHERE  is_productive=0 AND visit_date>=\''.$dateFrom.'\' AND visit_date<=\''.$dateTo.'\' AND user_id=\''.$user.'\' AND (user_id, outlet_id ,visit_date) NOT IN (SELECT user_id, outlet_id, visit_date FROM `tbl_trans_visit_log` WHERE is_productive=1 AND visit_date>=\''.$dateFrom.'\' AND visit_date<=\''.$dateTo.'\' AND user_id=\''.$user.'\' GROUP BY user_id, visit_date, outlet_id) GROUP BY user_id, visit_date, outlet_id) u ON m.outlet_id=u.unproductive_outlets AND m.visit_date=u.visit_date AND m.user_id=u.user_id GROUP BY m.user_id, m.visit_date, m.outlet_id) T WHERE T.visit_date>=\''.$dateFrom.'\' AND T.visit_date<=\''.$dateTo.'\' AND T.user_id=\''.$user.'\' GROUP BY T.user_id, T.visit_date');
        */
        $query=$this->db->query('SELECT M.user_id, M.`visit_date`,COUNT(M.outlet)  AS made_call_count,'
                . 'COUNT(IF(M.issuccess_call=1,1,NULL)) AS PC, COUNT(IF(M.issuccess_call=0,1,NULL)) AS UPC'
                . ' FROM (SELECT  `user_id`, `visit_date`,outlet_id,  COUNT(DISTINCT(`outlet_id`)) AS outlet,  IF(SUM(`is_productive`)=0,0,1 ) AS issuccess_call  FROM `tbl_trans_visit_log` WHERE  visit_date>=\''.$dateFrom.'\' AND  visit_date<=\''.$dateTo.'\' AND user_id=\''.$user.'\' GROUP BY user_id,visit_date,outlet_id) M WHERE M.visit_date>=\''.$dateFrom.'\' AND M.visit_date<=\''.$dateTo.'\' AND user_id=\''.$user.'\' GROUP BY M.user_id, M.visit_date');
        $result = $query->row();
        return $result;
    }
    //GPS DATA TAKING
    ...
    function calActualWorkings($fromDate, $toDate) {
        ...
        $this->db->where($arr);
        $this->db->where('invh.tot_a_val<>0 and invh.tot_a_val<>0.1 and invh.tot_a_val<>0.01');
    ...
    function getRepActualDayCount($from, $to) {
        ...
        $this->db->select('`ag_code`, IF(`range_name`=\'T\',\'D\',`range_name`) AS `range_name`, SUM(`working_count`) AS `actual_working_count`');
        $this->db->from('`tbl_mst_rep_calendar`');
        $this->db->where($arrWhere);
