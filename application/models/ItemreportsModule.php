...
');
    ...
    function getHardCountDaily($area, $range, $fromDate, $toDate) {
        ...
        $strRange='';
        if(!empty($range) && isset($range) && $range == 'D'){
            $strRange='AND ((invh.cd=\'' . $range . '\' AND tbl_mst_range.range_name=\'' . $range . '\') OR (invh.cd=\'T\' AND tbl_mst_range.range_name=\'T\')) ';
        }else{
            $strRange=' AND invh.cd=\'' . $range . '\' AND tbl_mst_range.range_name=\'' . $range . '\' ';
        }
...
');*/
            $q = $this->db->query(''
                . 'INSERT INTO `tbl_trans_hardcount_base`(`area_name`, `ag_name`, `ag_cd`, `cd`, `date_actual`, `Auto`, `CategoryPerBill`, `user_name`, `uniques_id`) SELECT tbl_mst_area.area_name,agency_mst.ag_name,invh.ag_cd,IF(invh.cd=\'T\',\'D\',invh.cd),invh.date_actual,shop_mst.Auto,COUNT(DISTINCT(tbl_mst_item_category.name)) AS CategoryPerBill,\''.$sess['username'].'\',concat(invh.ag_cd,\'-\',invh.cd,\'-\',invh.date_actual,\'-\',shop_mst.Auto,\'-\',\''.$sess['username'].'\') AS uniques_id FROM `invh`
INNER JOIN invd ON invh.invno=invd.invno 
INNER JOIN agency_mst ON invh.ag_cd=agency_mst.ag_cd   
...
INNER JOIN tbl_mst_range ON tbl_mst_geography.range_id=tbl_mst_range.id 
...
INNER JOIN tbl_mst_item_category_purpose ON tbl_mst_item_category_purpose_link_category.purpose_id=tbl_mst_item_category_purpose.id
INNER JOIN shop_mst ON invh.ag_cd=shop_mst.sh_ag_cd AND invh.ro_cd=shop_mst.sh_ro_cd AND invh.sh_cd=shop_mst.sh_cd
WHERE invh.date_actual>=\'' . $dateToCal . '\' AND invh.date_actual<=\'' . $dateToCal . '\'  '.$strRange.' AND tbl_mst_item_category_purpose.id=4 AND invh.b_a=\'A\' AND invh.tot_a_val<>0 ' . $str . ' 
GROUP BY invh.ag_cd,invh.cd,invh.date_actual,shop_mst.Auto;
');
        //$result = $q->result_array();
        //echo $this->db->last_query();
...
FROM tbl_trans_hardcount_base a WHERE user_name=\''.$sess['username'].'\' GROUP BY a.area_name,a.ag_name,a.ag_cd,a.cd');
    ...
    function getDailyItemWiseSale($range, $fromDate, $toDate) {
        ...
        $this->db->join('`tbl_mst_range`', '`tbl_mst_range_item`.`range_id`=`tbl_mst_range`.`id`', 'INNER');
        ...
        $this->db->where('`tbl_mst_range`.`range_name`',$range);
...
//        echo $this->db->last_query().'<br>';
    ...
    function getBookingVsActual($area, $range, $fromDate, $toDate) {
        
        $sess = $this->session->userdata('User');
        
        
        
        $this->db->select('tbl_mst_region.region_name,tbl_mst_area.area_name,tbl_mst_territory.territory_name,tbl_mst_item.item,tbl_mst_item.description,SUM(invd.a_Qty) AS booking_qty,SUM(IF(invh.b_a=\'A\',invd.a_Qty,0)) AS actual_qty');
        $this->db->from('`invh`');
        $this->db->join('`invd`', '`invh`.`invno`=`invd`.`invno`', 'INNER');
        ...
        $this->db->join('`tbl_mst_item`', '`tbl_mst_item`.`item`=`invd`.`item`', 'INNER');

        $this->db->where('`invh`.`date_book` >=', $fromDate);
        $this->db->where('`invh`.`date_book` <=', $toDate);
        $this->db->like('`tbl_mst_item`.`description` ','Nenaposha');

        $this->db->group_by('`tbl_mst_region`.`region_name`');
        $this->db->group_by('`tbl_mst_area`.`area_name`');
        $this->db->group_by('`tbl_mst_territory`.`territory_name`');
        $this->db->group_by('`tbl_mst_item`.`item`');

        $this->db->order_by('`tbl_mst_region`.`region_name`');
        $this->db->order_by('`tbl_mst_area`.`display_order`');
        $this->db->order_by('`tbl_mst_area`.`area_name`');
        $this->db->order_by('`tbl_mst_territory`.`territory_name`');
        $this->db->order_by('`tbl_mst_item`.`item`');
