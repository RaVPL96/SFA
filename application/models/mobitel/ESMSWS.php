...
function getMessagesFromLongNumber($session,$longNumber)
{
	...
	$getMessagesFromLongNumberResponse = new stdClass();
        if(!empty($getmessagesFromLongNumberResponse) && isset($getmessagesFromLongNumberResponse)){
...
print_r(serviceTest('','esmsusr_68vsnCdV','R2AqUhOk','')); echo '<br>';
echo isSession($session).'<br>';
//sendMessages($session,'Raigam','test',array('94707061772'), 0);
//getMessagesFromShortCode($session,"shortcode");
print_r($session);
print_r(getMessagesFromLongNumber($session,"94707062062"));
closeSession($session);
*/
?>
