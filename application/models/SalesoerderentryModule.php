...
class SalesoerderentryModule extends CI_Model {
    ...
    function getOrdersH($FromDate = '', $ToDate = '', $invNum = '', $audit_user = '', $channelList = null, $report_type = 'detail', $areaID = null, $territoryID = null, $rangeID = null) {
        ...
        if ($report_type == 'detail') {
            $this->db->select('`tbl_trans_order_h`.`id`, `bill_name`, `customer_id`, `address1`, `address2`, `address3`, `tbl_trans_order_h`.`contact_person`, `tbl_trans_order_h`.`mobile`, `tbl_trans_order_h`.`area_id`, `tbl_trans_order_h`.`territory_id`, `tbl_trans_order_h`.`route_id`, `subtotal`, `header_discount`, `header_discount_value`, `total_discount_value`, `header_cancel_value`, `header_gr_value`, `header_mr_value`, `header_net_value`, `invoice_category`, `inv_status`, `inv_type`, `inv_date`, `inv_time`, `audit_user`, `is_synced`, `tbl_trans_order_h`.`latitude`, `tbl_trans_order_h`.`longitude`, `distributor_id`, `distributor_stock_id`, `tbl_trans_order_h`.`channel_id`, `tbl_trans_order_h`.`operation_id`, `tbl_trans_order_h`.`range_id`, `app_inv_no`,`tbl_mst_territory`.`reference_code` AS `territory_code`,`tbl_mst_route`.`reference_code` AS `route_code`,`tbl_mst_route`.`route_name`,`tbl_mst_outlet`.`reference_code` AS `shop_code`,`inv_start_date`,`inv_start_time`,`tbl_mst_route_link_outlet`.`outlet_code`,(6371*acos(cos(radians(tbl_mst_outlet.latitude))*cos(radians(tbl_trans_order_h.latitude))*cos(radians(tbl_trans_order_h.longitude)-radians(tbl_mst_outlet.longitude))+sin(radians(tbl_mst_outlet.latitude))*sin(radians(tbl_trans_order_h.latitude))))as distance');
