...
ORDER BY tbl_mst_region.id,tbl_mst_area.area_name,agency_mst.ag_name,invh.ag_cd,cd');
    ...
    function getTargetNewDemarcation($tyear, $tmonth, $area_cd = NULL) {
        $this->db->select('`tbl_mst_area`.`id` AS `area_cd`, `target_value`.`ag_cd`, `t_year`, `t_mon`, `tbl_mst_area`.`area_name`, `ag_name`, `c_target`, `d_target`, `a_target`, `b_target`, `s_target`, `t_target`, `r_target`, `acs_pc_target`, `bd_pc_target`, `wd`, `p_wd`, `auto`');
        $this->db->from('`target_value`');
