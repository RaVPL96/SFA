...
ORDER BY area_h.area_name,agency_mst.ag_name,invh.ag_cd,cd');
...
GROUP BY invh.ag_cd,invh.date_actual ) a GROUP BY a.ag_cd';
    ...
    function getCategoryList($purpose_id,$cat_id=null) {
        $this->db->select('`tbl_mst_item_category`.`id`, `tbl_mst_item_category`.`name`, `tbl_mst_item_category`.`valid_from`, `tbl_mst_item_category`.`valid_to`, `tbl_mst_item_category`.`isact`,bg_color,font_color');
