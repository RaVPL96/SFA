...
class CronModule extends CI_Model {
    ...
    function createDefaultRangeItemRecord($ag_cd, $range_name) {
        $todt = date('Y-m-31', strtotime(date('Y-m') . " -1 month"));
        $count = 1;
        for ($j = 0; $j < 5; $j++) {
            if (date('m', strtotime(date('Y-m') . " -" . $j . " month")) == 4 || date('m', strtotime(date('Y-m') . " -" . $j . " month")) == 3) {
                $count += 2;
            } else {
                $count += 1;
            }
        }
        $fromdt = date('Y-m-01', strtotime(date('Y-m') . " -" . $count . " month")) . '<br>';
        $this->db->select('`tbl_mst_distributor_warehouse`.`id` AS `stock_location_id`');
        ...
        $this->db->where('`tbl_mst_distributor`.`agency_code`', $ag_cd);
        $q = $this->db->get();
        $r = $q->result_array();

        //get aavg sale last 6 moth without march april
        $avgR = $this->db->query('SELECT invd.item,SUM(invd.a_Qty)/6 as `avg_sale` FROM invh INNER JOIN invd ON invh.invno=invd.invno WHERE ag_cd=' . $ag_cd . ' AND invh.cd=\''.$range_name.'\' AND invh.date_actual>=\'' . $fromdt . '\' AND invh.date_actual<=\'' . $todt . '\' AND invh.invno NOT IN (SELECT  a.invno FROM invh a WHERE a.date_actual>=\'' . date('Y-03-01') . '\' AND a.date_actual<=\'' . date('Y-04-30') . '\' AND a.ag_cd=' . $ag_cd . ' AND invh.cd=\''.$range_name.'\') GROUP BY invd.item');
        $avgData = $avgR->result_array();

        if (!empty($r) && isset($r)) {
            foreach ($r as $l) {
                //records have need to update
                $this->db->select('`tbl_mst_range_item`.`item`');
                $this->db->from('`tbl_mst_range_item`');
                $this->db->where('`range_name`', $range_name);
                $qi = $this->db->get();
                $ri = $qi->result_array();
                if (!empty($ri) && isset($ri)) {//records have need to update
                    foreach ($ri as $k) {
                        ...
                        $this->db->where('`location_code`', $l['stock_location_id']);
                        $this->db->where('`item`', $k['item']);
                        $this->db->where('`unit_price`', 0);
                        $queryN = $this->db->get();
                        $resultN = $queryN->result_array();

                        //last 6 month avg sale
                        $avgSale = 0;
                        if (!empty($avgData) && isset($avgData)) {
                            foreach ($avgData as $value) {
                                if($k['item']==$value['item']){
                                    $avgSale = $value['avg_sale'];
                                }
                            }                            
                        }
                        ...
                        if (!empty($resultN) && isset($resultN)) {//records have need to update
                            ...
                            $arrIn = array(
                                '`location_code`' => $l['stock_location_id'],
                                '`item`' => $k['item'],
                                '`unit_price`' => 0,
                                '`op_sellable`' => 0,
                                '`op_damage`' => 0,
    ...
    function updateOpeningStock($ag_cd, $date_posted, $range_name) {//old agency code,applicable date
        ...
        $this->db->join('`tbl_mst_range_item`', '`stock_d`.`item`=`tbl_mst_range_item`.`item`', 'RIGHT');
        $this->db->where(array(
            ...
            '`tbl_mst_range_item`.`range_name`' => $range_name
        ));
        $query = $this->db->get();
    ...
    function updateSalesStock($ag_cd, $date_posted,$range_name) {//old agency code,applicable date
        $IsInserted = 1;
    ...
    ///////////////////////////////////////////////  PENDING ORDERS IN HEAD OFFICE  //////////////////////////////////////////
    function pendingOrdersInHeadOffice($ag_cd, $date_posted) {
        $IsInserted = 1;
        //get received head office invoice after stock taking
        $this->db->select('`tbl_mst_distributor_warehouse`.`id` AS `stock_location_id`,`tbl_mst_item_erp_codes`.`item_code` as `item`, `tbl_trans_company_ordd`.`up` AS `unit_price`,SUM(`tbl_trans_company_ordd`.`qty`*`tbl_mst_item_erp_codes`.`conversion_factor`) AS received_qty');
        $this->db->from('`tbl_mst_distributor_warehouse`');
        $this->db->join('`tbl_mst_distributor`', '`tbl_mst_distributor_warehouse`.`distributor_id`=`tbl_mst_distributor`.`id`', 'INNER');
        $this->db->join('`tbl_trans_company_ordh`', '`tbl_mst_distributor`.`agency_code`=`tbl_trans_company_ordh`.`ag_cd`', 'INNER');
        $this->db->join('`tbl_trans_company_ordd`', '`tbl_trans_company_ordh`.`c_invno`=`tbl_trans_company_ordd`.`c_invno`', 'INNER');
        $this->db->join('`tbl_mst_item_erp_codes`', '`tbl_trans_company_ordd`.`item_code`=`tbl_mst_item_erp_codes`.`erp_item_code`', 'INNER');
        $this->db->where(array(
            '`tbl_mst_distributor`.`agency_code`' => $ag_cd,
            '`tbl_mst_distributor`.`is_act`' => 1,
            '`tbl_trans_company_ordh`.`c_inv_date`>' => $date_posted
        ));
        $this->db->group_by('`tbl_mst_distributor_warehouse`.`id`,`tbl_mst_item_erp_codes`.`item_code`, `tbl_trans_company_ordd`.`up`');
        $query = $this->db->get();
        $result = $query->result_array();
        ...
        if (!empty($result) && isset($result)) {
            foreach ($result as $r) {
                ...
                if (!empty($resultN) && isset($resultN)) {//records have need to update
                    $arrUpdate = array(
                        '`head_office_active_ord_qty`' => $r['received_qty']
                    );
                    $this->db->where('`location_code`', $r['stock_location_id']);
                    $this->db->where('`item`', $r['item']);
                    $this->db->where('`unit_price`', $r['unit_price']);
    ...
    function getAgencyStocktoRecal() {
        ...
        if (!empty($results) && isset($results)) {// we have a agency to recal stock - call stock re calculation
            foreach ($results as $r) {
                ...
                echo '<br>addSalesGiven-Done' . $this->updateSalesStock($r['ad_code'], $r['stock_date'], $r['range_name']);
                ...
                // Pending orders in head office
                echo '<br>addSalesGiven-Done' . $this->pendingOrdersInHeadOffice($r['ad_code'], $r['stock_date']);

                echo '<br>create default item record if not item listed' . $this->createDefaultRangeItemRecord($r['ad_code'], $r['range_name']);
...
// option 1
    ...
    function resetBlobShopUpdate() {
        $sql2 = 'update tbl_trans_shop_update SET tbl_trans_shop_update.image=\'aa\' WHERE tbl_trans_shop_update.is_updated=1 AND tbl_trans_shop_update.image_created=1 AND tbl_trans_shop_update.image!=\'aa\';';
        $this->db->query($sql2);
    }
    ...
    function updateDefaultInvoice() {
        ...
        foreach ($resultb as $b) {
            ...
            /* print_r($b) */echo $inv = $b['range_name'] . sprintf('%03d', $b['reference_code']) . '00'.date('m');// die();
            echo '<br>';
            ...
            if (!empty($r->invno) && isset($r->invno)) {
                ...
                $arrUp = array(
                    '`date_book`' => date('Y-m-01'),
                    '`date_actual`' => date('Y-m-01'),
                    '`b_a`' => 'A',
                );
                $this->db->where('`invno`', $inv);
    ...
    function cronBorellaMaligawatta() {
        $sql2 = 'delete from `invh_cd_ratio`';
        $this->db->query($sql2);
        $sql = 'INSERT INTO `invh_cd_ratio`( `invno`, `c_ratio`, `d_ratio`) 
SELECT invd.invno, ROUND(SUM(IF(tbl_mst_range_item.range_name=\'C\',(invd.a_Qty*invd.up),0)) /(SUM(IF(tbl_mst_range_item.range_name=\'C\',(invd.a_Qty*invd.up),1)) + SUM(IF(tbl_mst_range_item.range_name=\'D\',(invd.a_Qty*invd.up),1))),2) AS c_ratio, ROUND(SUM(IF(tbl_mst_range_item.range_name=\'D\',(invd.a_Qty*invd.up),0)) /(SUM(IF(tbl_mst_range_item.range_name=\'C\',(invd.a_Qty*invd.up),1)) + SUM(IF(tbl_mst_range_item.range_name=\'D\',(invd.a_Qty*invd.up),1))),2) AS d_ratio 
FROM `invd` 
INNER JOIN invh ON invd.invno=invh.invno
INNER JOIN tbl_mst_range_item ON invd.item=tbl_mst_range_item.item
WHERE invh.cd =\'S\' AND (tbl_mst_range_item.range_name=\'C\' OR tbl_mst_range_item.range_name=\'D\') AND date_actual>=\'2023-12-01\'
AND invh.b_a=\'A\' and invh.tot_a_val<>0
GROUP BY `invd`.`invno`';
        $this->db->query($sql);
    }

    function approveShops($range){
        $sql = 'UPDATE `tbl_mst_outlet` `O` 
INNER JOIN tbl_mst_rep_link_territory_agent ON `O`.`created_by`=tbl_mst_rep_link_territory_agent.rep_username
SET `O`.`is_new`=0, `O`.`is_act`=1 
WHERE tbl_mst_rep_link_territory_agent.range_name=\''.$range.'\' AND `O`.created_date>=\'2024-09-04\';';
        $this->db->query($sql);
    }
}
