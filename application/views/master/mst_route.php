...
<div class="content-wrapper">
    ...
    <section class="content-header">
        <h1>
            Sales Geography - Route Architecture
            <small>Maintain Secondary Sales Route Details </small>
        </h1>
        <ol class="breadcrumb">
    ...
    <section class="content">
        <div class="row">
            <div class="col-md-6">
				...
				$isdelval = '';
                $btnmsg = 'Create Sales Route';
                if (!empty($RouteData) && isset($RouteData)) {
                    $routeName = $RouteData->route_name;
                    $lid = $RouteData->id;
					$routeOrder=$RouteData->display_order;
					$refCode = $RouteData->reference_code;
                    $isact = $RouteData->is_act;
					$isdel = $RouteData->is_del;
                    $readLvl = 'disabled=disabled';
                    $btnmsg = 'Update Sales Route';
                    if ($isact == 1) {
                        $isactval = 'checked="checked"';
                    } else {
                        $isactval = '';
                ...
                <form id="frmLocation" role="form" action="<?= base_url('index.php/master/saveRoute') ?>" method="post">
                    <fieldset>
                        <div class="box">
                            <div class="box-header with-border">
                                <h3 class="box-title">Route Geography Layer</h3> (Enter Route Layer Details)  <a class="btn btn-success pull-right btn-sm" href="<?= base_url('index.php/master/createRoute/') ?>"><i class="fa fa-plus"></i> New</a>                              
                            </div><!-- /.box-header -->
                            <div class="box-body">
								
                                <div class="form-group">
                                    <input type="text" name="sop[routeName]" class="form-control" value="<?= $routeName ?>" placeholder="Enter Name for Route architecture">
                                </div>
                                <div class="form-group">
                                    <input type="text" name="sop[routeCode]" class="form-control" value="<?= $refCode ?>" placeholder="Enter Code for Route architecture">
                                </div>
                                <div class="form-group">
                                    <input type="text" name="sop[routeOrder]" class="form-control" value="<?= $routeOrder ?>" placeholder="Enter Order for Route architecture">
                                </div>
                                <div class="form-group"> 
            ...
            <div class="col-md-6">
                <div class="box">
                    <div class="box-header with-border">
                        <h3 class="box-title">Sales Route List</h3> (Click Edit to change details)
                    </div><!-- /.box-header -->
                    <div class="box-body">
                        <table class="table table-bordered" id="example1">
                            ...
                            <tr>
                                ...
                                <th style="width: 180px">Route Order</th>
							...
							if(!empty($RouteDataSet) && isset($RouteDataSet)){
								foreach ($RouteDataSet as $loc) {
									...
									echo '<tr class="info">
											...
											<td><a class="btn btn-primary btn-xs" href="' . base_url('index.php/master/createRoute/' . $loc['id']) . '">Edit</a></td>
