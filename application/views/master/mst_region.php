...
<div class="content-wrapper">
    ...
    <section class="content-header">
        <h1>
            Sales Geography - Region Architecture
            <small>Maintain Secondary Sales Region Details </small>
        </h1>
        <ol class="breadcrumb">
    ...
    <section class="content">
        <div class="row">
            <div class="col-md-6">
				...
				$isdelval = '';
                $btnmsg = 'Create Sales Region';
                if (!empty($RegionData) && isset($RegionData)) {
                    $regionName = $RegionData->region_name;
                    $lid = $RegionData->id;
					$refCode = $RegionData->reference_code;
                    $isact = $RegionData->is_act;
					$isdel = $RegionData->is_del;
                    $readLvl = 'disabled=disabled';
                    $btnmsg = 'Update Sales Region';
                    if ($isact == 1) {
                        $isactval = 'checked="checked"';
                    } else {
                        $isactval = '';
                ...
                <form id="frmLocation" role="form" action="<?= base_url('index.php/master/saveRegion') ?>" method="post">
                    <fieldset>
                        <div class="box">
                            <div class="box-header with-border">
                                <h3 class="box-title">Region Geography Layer</h3> (Enter Region Layer Details)  <a class="btn btn-success pull-right btn-sm" href="<?= base_url('index.php/master/createRegion/') ?>"><i class="fa fa-plus"></i> New</a>                              
                            </div><!-- /.box-header -->
                            <div class="box-body">
								
                                <div class="form-group">
                                    <input type="text" name="sop[regionName]" class="form-control" value="<?= $regionName ?>" placeholder="Enter Name for Region architecture">
                                </div>
                                <div class="form-group">
                                    <input type="text" name="sop[regionCode]" class="form-control" value="<?= $refCode ?>" placeholder="Enter Code for Region architecture">
                                </div>
                                <div class="form-group"> 
            ...
            <div class="col-md-6">
                <div class="box">
                    <div class="box-header with-border">
                        <h3 class="box-title">Sales Region List</h3> (Click Edit to change details)
                    </div><!-- /.box-header -->
                    <div class="box-body">
							...
							if(!empty($RegionDataSet) && isset($RegionDataSet)){
								foreach ($RegionDataSet as $loc) {
									...
									echo '<tr class="info">
											...
											<td><a class="btn btn-primary btn-xs" href="' . base_url('index.php/master/createRegion/' . $loc['id']) . '">Edit</a></td>
