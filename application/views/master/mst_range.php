...
<div class="content-wrapper">
    ...
    <section class="content-header">
        <h1>
            Sales Range Architecture
            <small>Maintain Secondary Sales Range Details </small>
        </h1>
        <ol class="breadcrumb">
    ...
    <section class="content">
        <div class="row">
            <div class="col-md-6">
				...
				$isdelval = '';
                $btnmsg = 'Create Sales Range';
                if (!empty($RangeData) && isset($RangeData)) {
                    $rangeName = $RangeData->range_name;
                    $lid = $RangeData->id;
					//$oplid = $ZoneData->op_id;
                    $isact = $RangeData->is_act;
					$isdel = $RangeData->is_del;
                    $readLvl = 'disabled=disabled';
                    $btnmsg = 'Update Sales Range';
                    if ($isact == 1) {
                        $isactval = 'checked="checked"';
                    } else {
                        $isactval = '';
                    ...
                    <fieldset>
                        <div class="box">
                            <div class="box-header with-border">
                                <h3 class="box-title">Range Layer</h3> (Enter Range Layer Details)  <a class="btn btn-success pull-right btn-sm" href="<?= base_url('index.php/master/createRange/') ?>"><i class="fa fa-plus"></i> New</a>                              
                            </div><!-- /.box-header -->
                            <div class="box-body">
								
                                <div class="form-group">
                                    <input type="text" name="sop[rangeName]" class="form-control" value="<?= $rangeName ?>" placeholder="Enter Name for channel architecture">
                                </div>
                                <div class="form-group"> 
            ...
            <div class="col-md-6">
                <div class="box">
                    <div class="box-header with-border">
                        <h3 class="box-title">Sales Range List</h3> (Click Edit to change details)
                    </div><!-- /.box-header -->
                    <div class="box-body">
                        <table class="table table-bordered" id="example1">
							...
							<tbody>
                            <?php
							if(!empty($RangeDataSet) && isset($RangeDataSet)){
								foreach ($RangeDataSet as $loc) {
									$action = 0;
									...
									echo '<tr class="info">
											...
											<td><a class="btn btn-primary btn-xs" href="' . base_url('index.php/master/createRange/' . $loc['id']) . '">Edit</a></td>
