...
<div class="content-wrapper">
    ...
    <section class="content" style="overflow:hidden;">
        ...
        <div class="row">
            <div class="col-md-12">  
                <form class="form-horizontal" id="myForm" action="<?= base_url('Itemreports/shopSale') ?>" method="post">
                    <div class="col-md-4">                            
                    ...
                    <div class="col-md-4">                            
                        <div class="form-group">
                            <label class="col-md-4">Criteria <span class="text-red">*</span></label>
                            <div class="col-md-6">
                                <select id="sbValueRange" name="sbValueRange" class="form-control">
                                    <option value=">="> -- Select Value Range -- </option>    
                                    <option <?php
                                    if ($sbValueRange == '>=') {
                                        echo 'selected';
                                    }
                                    ?> value=">=">Greater than or equal to (වඩා විශාල හෝ සමාන)</option>    
                                    <option <?php
                                    if ($sbValueRange == '<=') {
                                        echo 'selected';
                                    }
                                    ?> value="<=">Less than or equals to (වඩා අඩු හෝ සමාන)</option>    
                                    <option <?php
                                        if ($sbValueRange == '=') {
                                            echo 'selected';
                                        }
                                    ?> value="=">Equal to (සමාන )</option>    
                                </select>
                            </div> 
                        </div>
                    </div>
                    <div class="col-md-4">                            
                        <div class="form-group">
                            <label class="col-md-4">Value of Rs. <span class="text-red">*</span></label>                            
                            <div class="col-md-6">
                                <input type="text" name="ValuePoint" class="form-control pull-right active col-md-12" id="valuePoint" value="<?= $ValuePoint ?>">
                            </div>
                        </div>
                    </div>
                    <div class="col-md-5">
            ...
            <div class="col-md-12" style="overflow-x: scroll; overflow-y: scroll; max-height: 700px;">
                    ...
                    if (!empty($DataSet) && isset($DataSet)) {
                        ?>
                    <h2>Shop Sale Value Status Category-wise <?php
                        $catN = '';
                    ...
                    <table id="attendance_table" class="table table-hover presentation">
                        <thead>
                            <tr>
                                ...
                                foreach ($daterange as $date) {
                                    //$lines.= '<td>'.${'d[\'value_'.$date->format("Y_m_d").'\']'}.'</td>';
                                    $headings .= '<th>Value ' . $date->format("Y-m-d") . '</th>';
                                }
                                echo $headings;
                                ?>
                        ...
                        foreach ($DataSet as $d) {
                            $lines = '';
                            foreach ($daterange as $date) {
                                if ($d['value_' . $date->format("Y_m_d")] == 0) {
                                    $lines .= '<td> </td>';
                                } else {
                                    $lines .= '<td>' . number_format($d['value_' . $date->format("Y_m_d")]) . '</td>';
                                }
                            }
