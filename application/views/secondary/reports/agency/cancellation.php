        ...
        <div class="row">            
            <div class="col-md-12">               
                ...
                <div class="row" style="overflow-x: scroll;">   
                    <div class="col-md-12">                 
                        <form class="form-horizontal" id="OrderData" action="<?= base_url('salesreports/cancellation') ?>" method="post" novalidate="novalidate">             
                            <div class="col-md-6">       
                    ...
                    <div class="col-md-10">
                        ...
                        if (!empty($dataInfo) && isset($dataInfo)) {
                            ...
                            <table class="table table-hover">
                                <thead>
                                    <tr>
                                        <th>Total Full Cancel Invoices </th>
                                ...
                                <tbody>
                                    <?php foreach ($dataInfo As $row) {
                                        ...
                                        <tr>
                                            ...
                                            <td> <?= number_format($row['total_cancel_value'], 2) ?></td>
                                            <td> <?= number_format($row['total_booking_value'], 2) ?></td>
                                            <td> <?= number_format($row['total_cancel_value'] / $row['total_booking_value'] * 100, 2) ?>%</td>
                                        </tr>
                                    <?php }
                                    ?>
                                </tbody>
                        ...
                        if (!empty($territoryData) && isset($territoryData)) {
                            ...
                            <table class="table table-hover">
                                ...
                                <tbody>
                                    <?php foreach ($territoryData as $r) {
                                        ?>
                                        <tr>
                                            <td> <?= $r['territory_name'] ?></td>
                                            <td align="right"> <?= $r['total_count'] ?></td>
                                            <td align="right"> <?= number_format($r['total_cancel_value'], 2) ?></td>
                                            <td align="right"> <?= number_format($r['total_booking_value'], 2) ?></td>
                                            <td align="right"> <?= number_format($r['total_cancel_value'] / $r['total_booking_value'] * 100, 2) ?>%</td>
                                        </tr>
                                    <?php } ?>
                                </tbody>
                            </table>
                            ...
                            <?php
                            if (!empty($detailInfo) && isset($detailInfo)) {
                                ?>
                                <table class="table table-hover">
                                    <thead>
                                        <tr>
                                    ...
                                    <tbody>
                                        ...
                                        foreach ($detailInfo as $row) {
                                            $colorSet = '';
                                            if ($row['tot_a_val'] == 0) {
                                                $colorSet = 'style="background: #faa19b;"';
                                            }
                                            ?>
                                            <tr <?= $colorSet ?> >
                                                <td> <?= $row['invno'] ?></td>
                                                <td> <?= $row['name'] ?> </td>
                                                <td> <?= sprintf('%03d',$row['ag_cd']).'/'.sprintf('%02d',$row['ro_cd']).'/'.sprintf('%03d',$row['sh_cd']) ?> </td>
                                                <td align="right"> <?= number_format($row['tot_b_val'], 2) ?></td>
                                                <td align="right"> <?= number_format($row['tot_c_val'], 2) ?></td>
                                                <td align="right"> <?= number_format($row['tot_a_val'], 2) ?></td>
                                                <td> <?= $row['date_book'] ?></td>
                                                <td> <?= $row['date_actual'] ?></td>
                                                
                                                <td><a href="#" onclick="view_invoice_old('<?= $row['invno'] ?>'); return false;" style="cursor: pointer;">View</a></td>
                                            </tr>
        <?php } ?>
                                    </tbody>
                                </table> 
