<?php include_once BASEPATH . ('../application/views/template/leftsidebar.php'); ?>
<!-- Content Wrapper. Contains page content -->
<div class="content-wrapper">
    <!-- Content Header (Page header) -->
    <section class="content-header">
        <h1>
            Secondary Sales Customers
            <small>Maintain Secondary Sales Details </small>
        </h1>
        <ol class="breadcrumb">
            <li><a href="#"><i class="fa fa-dashboard"></i> Home</a></li>
            <li class="active">Master Data Control Module</li>
        </ol>
    </section>

    <!-- Main content -->
    <section class="content">
        <div class="row">
            <div class="col-md-12" style="overflow-x: scroll;">
                <div id="successMessage" class="hideDiv">Updated Successfully! </div>
                <div id="errorMessage" class="hideDiv">Invalid Data! Please Try Again. </div>
                <div class="keepgap"></div> 
                <?php
                $id = '';
                $name = '';
                $address4 = $address3 = $address2 = $address = '';
                $contact_person = '';
                $telephone = '';
                $mobile = '';
                $outlet_category_id = '';

                $dealercode = '';
                $territory_reference_code = '';
                $route_reference_code = '';
                $isact = '';
                $isapprove = '';
                $image = '';

                $address1_up = '';
                $address2_up = '';
                $address3_up = '';
                $mobile_up = '';
                $image_up = '';
                $outlet_category_id_up = '';
                $allowed_to = '';
                $allowed_to_up = '';

                $is_approve = '';

                if (!empty($outlets) && isset($outlets)) {
                    $id = $outlets->id;
                    $name = $outlets->name;
                    $address = $outlets->address_1;
                    $address2 = $outlets->address_2;
                    $address3 = $outlets->address_3;
                    $contact_person = $outlets->contact_person;
                    $telephone = $outlets->telephone;
                    $mobile = $outlets->mobile;
                    if ($outlets->is_act == 1) {
                        $isact = 'checked';
                    }
                    $dealercode = $outlets->outlet_code;
                    $territory_reference_code = $outlets->territory_reference_code;
                    $outlet_category_id = $outlets->outlet_category_id;
                    $route_reference_code = $outlets->route_reference_code;
                    if ($outlets->is_new == 0) {
                        $isapprove = 'checked';
                    }
                    $image = $outlets->image;
                    $allowed_to = $outlets->allowed_to;

                    $address1_up = $outlets->address_1_up;
                    $address2_up = $outlets->address_2_up;
                    $address3_up = $outlets->address_3_up;
                    $mobile_up = $outlets->telephone_up;
                    $image_up = $outlets->image_up;
                    $outlet_category_id_up = $outlets->shop_type_up;
                    echo $allowed_to_up = $outlets->contact_person_up;
                }
                ?>

                <div class="col-md-12">
                    <?php if (!empty($outlets) && isset($outlets)) { ?>

                        <table id="example2" class="table table-hover">	
                            <thead>						
                                <tr>			
                                    <th>Image</th> 
                                    <th>Distance</th>
                                    <th>ID</th>
                                    <!-- <th>Reference Code</th> -->
                                    <th>Name</th> 		
                                    <th>Address 1</th> 	
                                    <th>Address 2</th> 	
                                    <th>Address 3</th> 	  
                                    <th>Mobile</th> 	
                                    <th>Shop Type</th> 	
                                    <th>Created Date</th> 	
                                    <th>Created By</th> 	
                                    <th>Display Order</th>  	
                                    <th>Active</th> 	
                                    <th>New</th> 		
                                </tr>		
                            </thead>		
                            <tbody>	
                                <?php
                                if (!empty($outlets) && isset($outlets)) {
                                    ?>

                                    <tr style="background: #ffff6e;">	
                                        <td><img style="width:150px;" src="data:image/jpeg;base64,<?= ($outlets->image_up) ?>"/></td> 
                                        <td>0 m</td> 	
                                        <td><?= $outlets->territory_reference_code . '/' . $outlets->route_reference_code . '/' . $outlets->outlet_code ?></td> 
                                        <!-- <td><?= $outlets->reference_code ?></td> -->
                                        <td><?= $outlets->name ?></td> 		
                                        <td><?= $outlets->address_1 ?></td> 	
                                        <td><?= $outlets->address_2 ?></td> 	
                                        <td><?= $outlets->address_3 ?></td>   	
                                        <td><?= $outlets->mobile ?></td> 		
                                        <td><?= $outlets->shop_type_name ?></td> 	
                                        <td><?= $outlets->created_date ?></td> 	
                                        <td><?= $outlets->created_by ?></td> 	
                                        <td><?= $outlets->display_order ?></td>   	
                                        <td><?= $outlets->is_act ?></td> 		
                                        <td><?= $outlets->is_new ?></td> 		

                                    </tr>
                                    <?php
                                }
                                ?>
                                <?php
                                $duplicate = '';
                                foreach ($outletsNearby as $o) {
                                    if ($o['duplicate_of'] != '-' && $o['duplicate_of'] == $o['id']) {//shop is a duplicate shop of a already created one
                                        $duplicate .= '<option value="' . $o['id'] . '">' . $o['name'] . '(' . $o['territory_reference_code'] . '/' . $o['route_reference_code'] . '/' . $o['outlet_code'] . ') in distance of ' . number_format($o['distance'] * 1000) . 'm</option>';
                                    }
                                    ?>
                                    <tr>
                                        <td><img style="width:150px;" src="data:image/jpeg;base64,<?= ($o['image']) ?>"/></td> 
                                        <td><?= (int) ($o['distance'] * 1000) ?> m</td> 	
                                        <td><a href="<?= base_url('salesarcustomers/editCustomers/' . $o['id']) ?>"><?= $o['territory_reference_code'] . '/' . $o['route_reference_code'] . '/' . $o['outlet_code'] ?></a></td> 
                                        <!-- <td><?= $o['reference_code'] ?></td> -->
                                        <td><?= $o['name'] ?></td> 		
                                        <td><?= $o['address_1'] ?></td> 	
                                        <td><?= $o['address_2'] ?></td> 	
                                        <td><?= $o['address_3'] ?></td>   	
                                        <td><?= $o['mobile'] ?></td> 		
                                        <td><?= $o['shop_type_name'] ?></td> 	
                                        <td><?= $o['created_date'] ?></td> 	
                                        <td><?= $o['created_by'] ?></td> 	
                                        <td><?= $o['display_order'] ?></td>   	
                                        <td><?= $o['is_act'] ?></td> 		
                                        <td><?= $o['is_new'] ?></td> 		

                                    </tr>						
                                    <?php
                                }
                                ?>                    			
                            </tbody> 					
                        </table>					
                        <?php
                    }
                    ?> 

                </div>	
            </div>

            <div class="col-md-12">
                <div id="map" style="width:90%;height:600px;"></div> 
            </div>
            <div class="col-md-12">
                <div class="col-md-12">
                    <form class="form-horizontal" id="frmLocation" action="<?= base_url('salesarcustomers/saveUpdatedOutlet') ?>" method="post" novalidate>
                        <table class="table table-hover">
                            <thead>
                                <tr>
                                    <th colspan="2" class="text-center"><h3><?= ucwords($name) ?></h3><h3>(<?= $territory_reference_code . '/' . $route_reference_code . '/' . $dealercode ?>)</h3></th> 
                                </tr>
                                <tr>
                                    <th><h3>New Details</h3></th>
                                    <th><h3>Old Details</h3></th>
                                </tr>
                            </thead>
                            <tbody>
                                <tr>
                                    <td>
                                        <img style="width:250px;" src="data:image/jpeg;base64,<?= $image_up ?>"/>
                                    </td>
                                    <td>
                                        <?php
                                        if ($image == 'aa') {
                                            $image = '';
                                        }
                                        ?>
                                        <img style="width:250px;" src="data:image/jpeg;base64,<?= $image ?>"/>
                                    </td>
                                </tr>
                                <tr>
                                    <td class="form-group"> 
                                        <label class="form-label">Outlet Address 1</label>
                                        <input class="form-control" type="text" name="outlet[address_1]" value="<?= ucfirst($address1_up) ?>">

                                    </td>
                                    <td>
                                        <label class="form-label"> </label>
                                        <input disabled="" class="form-control" type="text" name="outlet[address_1_old]" value="<?= ucfirst($address) ?>">
                                    </td>
                                </tr>
                                <tr>
                                    <td class="form-group"> 
                                        <label class="form-label">Outlet Address 2</label>
                                        <input class="form-control" type="text" name="outlet[address_2]" value="<?= ucfirst($address2_up) ?>">
                                    </td>
                                    <td><label class="form-label"> </label>
                                        <input disabled="" class="form-control" type="text" name="outlet[address_2_old]" value="<?= ucfirst($address2) ?>">
                                    </td>
                                </tr>
                                <tr>
                                    <td class="form-group">
                                        <label class="form-label">Outlet Address 3</label>
                                        <input class="form-control" type="text" name="outlet[address_3]" value="<?= ucfirst($address3_up) ?>">
                                    </td>
                                    <td>
                                        <label class="form-label"> </label>
                                        <input disabled="" class="form-control" type="text" name="outlet[address_3_old]" value="<?= ucfirst($address3) ?>">
                                    </td>
                                </tr>
                                <tr>
                                    <td class="form-group">
                                        <label class="form-label">Mobile</label>
                                        <input class="form-control" type="text" name="outlet[mobile]" value="<?= $mobile_up ?>">

                                    </td>
                                    <td>
                                        <label class="form-label"> </label>
                                        <input disabled="" class="form-control" type="text" name="outlet[mobile_old]" value="<?= $mobile ?>">
                                    </td>
                                </tr>
                                <tr>
                                    <td class="form-group"> 
                                        <label class="form-label">Outlet Category</label>
                                        <select name="outlet[category]" class="form-control">
                                            <?php
                                            foreach ($OutletCategory as $c) {
                                                $select = '';
                                                if ($c['id'] == $outlet_category_id_up) {
                                                    $select = ' selected ';
                                                }
                                                echo '<option ' . $select . ' value="' . $c['id'] . '">' . $c['name'] . '</option>';
                                            }
                                            ?>
                                        </select> 
                                    </td>
                                    <td> 
                                        <label class="form-label"> </label>
                                        <select disabled="" name="outlet[category_old]" class="form-control">
                                            <?php
                                            foreach ($OutletCategory as $c) {
                                                $select = '';
                                                if ($c['id'] == $outlet_category_id) {
                                                    $select = ' selected ';
                                                }
                                                echo '<option ' . $select . ' value="' . $c['id'] . '">' . $c['name'] . '</option>';
                                            }
                                            ?>
                                        </select> 
                                    </td>
                                </tr>
                                <tr>
                                    <td class="form-group">
                                        <label class="form-label">Outlet Range</label>
                                        <select name="outlet[range]" class="form-control">
                                            <?php
                                            $cd = $c = $d = '';
                                            if ($allowed_to_up == 'CD') {
                                                $cd = ' selected="selected" ';
                                            }
                                            if ($allowed_to_up == 'C') {
                                                $c = ' selected="selected" ';
                                            }
                                            if ($allowed_to_up == 'D') {
                                                $d = ' selected="selected" ';
                                            }
                                            echo '<option value="">--Select One--</option>';
                                            echo '<option ' . $cd . ' value="CD">CD</option>';
                                            echo '<option ' . $c . ' value="C">C Only</option>';
                                            echo '<option ' . $d . ' value="D">D Only</option>';
                                            ?>
                                        </select>	
                                    </td>
                                    <td>
                                        <label class="form-label">Outlet Range</label>
                                        <select disabled="" name="outlet[range_old]" class="form-control">
                                            <?php
                                            $cd = $c = $d = '';
                                            if ($allowed_to == 'CD') {
                                                $cd = ' selected="selected" ';
                                            }
                                            if ($allowed_to == 'C') {
                                                $c = ' selected="selected" ';
                                            }
                                            if ($allowed_to == 'D') {
                                                $d = ' selected="selected" ';
                                            }
                                            echo '<option value="">--Select One--</option>';
                                            echo '<option ' . $cd . ' value="CD">CD</option>';
                                            echo '<option ' . $c . ' value="C">C Only</option>';
                                            echo '<option ' . $d . ' value="D">D Only</option>';
                                            ?>
                                        </select>	
                                    </td>
                                </tr>
                                <tr style="background: #ff9898;">
                                    <td>
                                        <label class="form-label">If already created same outlet, select previously created outlet name.(දැනටමත් එම අලෙවිසැල නිර්මාණය කර ඇත්නම්, කලින් සාදන ලද අලෙවිසැලේ නම තෝරන්න)</label>
                                        <select name="outlet[duplicate_of]" class="form-control">
                                            <option value="-">Select if duplicate</option>
                                            <?= $duplicate ?>
                                        </select>
                                    </td>
                                    <td></td>
                                </tr>
                                <tr>
                                    <td class="form-group">
                                        <div class="col-md-6"><label class="form-label"><input  <?= $is_approve ?> type="radio" name="outlet[approve]" value="1"> Approve Shop Details</label></div>
                                        <div class="col-md-6"><label class="form-label"><input  <?= $is_approve ?> type="radio" name="outlet[approve]" value="2"> Reject Shop Details</label></div>
                                    </td>
                                    <td></td>
                                </tr>
                                <tr>
                                    <!--
                                    <td><input class="btn btn-info" type="submit" name="Save" value="Save"></td>
                                    <td></td>
                                    -->
                                </tr>
                            </tbody>
                        </table>

                        <div class="form-group"> 

                            <input class="form-control" type="hidden" name="outlet[name]" value="<?= ucwords($name) ?>">
                            <input type="hidden" name="outlet[id]" value="<?= $id ?>">
                            <input type="hidden" name="outlet[image_up]" value="<?= $image_up ?>">
                        </div>  

                    </form>
                </div> 
            </div>
            <div class="col-md-12">
                <script src="https://www.gstatic.com/external_hosted/jquery2.min.js"></script>                
                <script src="<?= base_url('adminlte/dist/js/validate/jquery.validate.min.js') ?>" type="text/javascript"></script>
                <script src="<?= base_url('adminlte/dist/js/validate/placeholders.min.js') ?>" type="text/javascript"></script>
                <script src="https://maps.googleapis.com/maps/api/js?v=3.exp&sensor=false&libraries=geometry&key=AIzaSyDL5TFORfvAKrvixbuRLQWhqfNGLL8sFCY"></script> 
                <script>

                    $("#frmLocation").validate({

                        rules: {
                            "outlet[address_1]": {
                                required: true
                            },
                            "outlet[address_2]": {
                                required: true
                            },
                            "outlet[address_3]": {
                                required: true
                            },
                            "outlet[mobile]": {
                                required: true
                            },
                            "outlet[category]": {
                                required: true
                            },
                            "outlet[range]": {
                                required: true
                            },
                            "outlet[approve]": {
                                required: true
                            }
                        },

                        messages: {

                            "outlet[address_1]": 'Please fill this field',

                            "outlet[address_2]": 'Please fill this field',

                            "outlet[address_3]": 'Please fill this field'

                        },

                        highlight: function (element) {

                            $(element).closest('.form-group').addClass('has-error');

                        },

                        unhighlight: function (element) {

                            $(element).closest('.form-group').removeClass('has-error');

                        },

                        errorElement: 'span',

                        errorClass: 'help-block',

                        errorPlacement: function (error, element) {

                            if (element.parent('.input-group').length) {

                                error.insertAfter(element.parent());

                            } else {

                                error.insertAfter(element);

                            }

                        },

                        /*submitHandler: function(form) {
                         
                         return true;
                         
                         }*/

                    });

                </script>

                <script type="text/javascript">
                    var center_lat = 0;
                    var center_lang = 0;
                    var locations = [
<?php
$str1 = '';
$str2 = '';
echo '[\'' . $outlets->name . '-' . 0 . ' m\', ' . $outlets->latitude_up . ', ' . $outlets->longitude_up . ', 12],';
if (!empty($outletsNearby) && isset($outletsNearby)) {

    foreach ($outletsNearby as $h) {
        if ((int) $h['distance'] == 0) {
            $str1 = 'center_lat=' . $h['latitude'] . ';';
            $str2 = 'center_lang=' . $h['longitude'] . ';';
        }
        echo '[\'' . str_replace('\'', '-', $h['name']) . '-' . ((int) ($h['distance'] * 1000)) . ' m\', ' . $h['latitude'] . ', ' . $h['longitude'] . ', 12],';
    }
}
?>
                    ];
<?php
echo $str1;
echo $str2;
?>
                    /*var locations = [
                     ['Bondi Beach', -33.890542, 151.274856, 4],
                     ['Coogee Beach', -33.923036, 151.259052, 5],
                     ['Cronulla Beach', -34.028249, 151.157507, 3],
                     ['Manly Beach', -33.80010128657071, 151.28747820854187, 2],
                     ['Maroubra Beach', -33.950198, 151.259302, 1]
                     ];*/

                    var map = new google.maps.Map(document.getElementById('map'), {
                        zoom: 15,
                        center: new google.maps.LatLng(<?= $outlets->latitude_up ?>, <?= $outlets->longitude_up ?>),
                        mapTypeId: google.maps.MapTypeId.ROADMAP
                    });

                    var infowindow = new google.maps.InfoWindow();

                    var marker, i;
<?php echo 'var icon_url=\'' . base_url('adminlte/dist/img/special_map_marker.png') . '\';'; ?>

                    marker = new google.maps.Marker({
                        position: new google.maps.LatLng(<?= $outlets->latitude_up ?>, <?= $outlets->longitude_up ?>),
                        map: map,
                        icon: icon_url
                    });

                    for (i = 0; i < locations.length; i++) {
                        if (locations[i][1] ==<?= $outlets->latitude_up ?> && locations[i][2] ==<?= $outlets->longitude_up ?>) {
                            marker = new google.maps.Marker({
                                position: new google.maps.LatLng(locations[i][1], locations[i][2]),
                                map: map,
                                icon: icon_url
                            });
                        } else {
                            marker = new google.maps.Marker({
                                position: new google.maps.LatLng(locations[i][1], locations[i][2]),
                                map: map
                            });
                        }


                        google.maps.event.addListener(marker, 'click', (function (marker, i) {
                            return function () {
                                infowindow.setContent(locations[i][0]);
                                infowindow.open(map, marker);
                            }
                        })(marker, i));
                    }
                </script>


            </div>
    </section>
    <!-- /.content -->
</div><!-- /.content-wrapper -->


