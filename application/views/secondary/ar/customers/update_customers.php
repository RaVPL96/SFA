...
if (!empty($outletsUpdateProgress) && isset($outletsUpdateProgress)) {
    ...
    <table id="attendance_table" class="table table-hover presentation">
        ...
        <tbody>
            ...
            $totalCloseCount = null;
            $totalSalesRepComplete = null;
            $totalPendingToApproval = null;
            $totalASEApproved = null;
            $totalDayBefore = null;
            $totalYesterday = null;
            foreach ($outletsUpdateProgress as $p) {
                $totalCount += $p['TOTAL'];
                $totalActiveCount += $p[ 'Active_count'];
                $totalCloseCount += $p['closed_count'];
                $totalSalesRepComplete += $p['COMPLETED'];
                $totalPendingToApproval += $p['PENDING_TO_COMPLETE'];
                $totalASEApproved += $p['PENDINGTO_APPROVED'];
                $totalDayBefore += $p['day_before_yesterday'];
                $totalYesterday += $p['yesterday'];
                ?> 
                <tr>
                    <?php
                    if (!empty($Area_ID) && isset($Area_ID)) {
                    ...
                    <!--<td><?= number_format((  $p['COMPLETED'] /($p['Active_count'] )   ) * 100) ?>%</td>-->
                   <td>
                        <?=
                        $p['Active_count'] > 0 
                            ? number_format(($p['COMPLETED'] / $p['Active_count']) * 100) . '%' 
                            : '0%'; 
                        ?>
                    </td>
                    ...
                    <td style="background: #c3c3c3;">
                        <div class="progress sm">
                            <div class="progress-bar progress-bar-aqua" style="width: <?= $p['Active_count']>0 ? number_format((  $p['COMPLETED'] / ($p['Active_count'] ) ) * 100) . '%' : '0%' ?>"></div>
                        </div>
                    </td>
            ...
            ?>
            <tr>
                <td style="background:lightyellow">Totals :</td>
                <td style="background:lightgreen">All</td>
                <td style="background:lightgreen"><span class="progress-text"><?= number_format($totalCount) ?></span></td>
                <td style="background:lightgreen"><span class="progress-text"><?= number_format($totalActiveCount) ?></span></td>
                <td style="background:lightgreen"><span class="progress-text"><?= number_format($totalCloseCount) ?></td>
                <td style="background:lightgreen"><span class="progress-text"><?= number_format($totalSalesRepComplete) ?></td>
                <td style="background:lightgreen"><?= $totalActiveCount>0 ? number_format((  $totalSalesRepComplete /$totalActiveCount  ) * 100) . '%' : '0%' ?></td>
                <td style="background:lightgreen"><span class="progress-text"><?= number_format($totalPendingToApproval) ?></td>
                <td style="background:lightgreen"><span class="progress-text"><?= number_format($totalASEApproved) ?></td>
                <td style="background:lightgreen">
                    <div class="progress sm">
                            <div class="progress-bar progress-bar-aqua" style="width: <?= $totalActiveCount>0 ? number_format(($totalSalesRepComplete/ $totalActiveCount ) * 100) . '%' : '0%' ?>"></div>
                        </div>
                </td>
                <td style="background:lightgreen"><span class="progress-text"><?= number_format($totalDayBefore) ?></span></td>
                <td style="background:lightgreen"><span class="progress-text"><?= number_format($totalYesterday) ?></span></td>
            </tr>
        </tbody>
    </table>
    </div>
