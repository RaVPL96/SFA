                            ...
                            <tbody>
                                ...
                                foreach ($DailySales as $v) {
                                    ...
                                    for ($n = 1; $n <= date('t', strtotime($datepickermonth)); $n++) {
                                        ...
                                        } else {// new area need to reset total
                                        }
                                    ...
                                    for ($k = 1; $k <= 6; $k++) {
                                        ...
                                        //Final Total    
                                        if (${'totWD' . $k} == 0) {
                                            $strBackFoot .= '<td class="text-right">' . number_format(${'totF' . $k}) . '</td><td>NA</td><td>' . ${'totPC' . $k} . '</td><td>' . ${'totWD' . $k} . '</td>';
                                        } else {
                                            $strBackFoot .= '<td class="text-right">' . number_format(${'totF' . $k}) . '</td><td>' . number_format(${'totPC' . $k} / ${'totWD' . $k}, 1) . '</td><td>' . ${'totPC' . $k} . '</td><td>' . ${'totWD' . $k} . '</td>';
                                        }
                                        if ($areaNameCurrent == $v['area_name']) {//we are in same area
                                            //AREA TOTAL
                                    ...
                                    } else {//switched to a new area so rest values
                                        //$AreatotVarienceValue =  $varienceValue; //AREA TOTAL
                                    }
...
//PRINT AREA TOTAL AT THE END
                                    if ($areaNameCurrent == $v['area_name']) {//we are in same area
                                        ...
                                        if (!empty($AreaID) && isset($AreaID) && $AreaID == '-1') {//all island report requested
                                            ...
                                            for ($k = 1; $k <= 6; $k++) {
                                                $strBackFootArea .= '<td class="text-right">' . number_format(${'totFArea' . $k}) . '</td><td>'. number_format(${'totPCArea' . $k}/${'totWDArea' . $k}).'</td><td>' . number_format(${'totPCArea' . $k}) . '</td><td>' . number_format(${'totWDArea' . $k}) . '</td>';
                                                ${'totFArea' . $k} = 0;
                                            }
                                    ...
                                    if ($areaNameCurrent == $v['area_name']) {//we are in same area
                                        if (count($DailySales) == $currentRow) {///we are in the last row 
                                            for ($n = 1; $n <= date('t', strtotime($datepickermonth)); $n++) {
                                                $AreastrRowFootWithPC .= '<td class="text-right">' . number_format(${'tot' . sprintf("%02d", $n) . '_area'}) . '</td>'
                                                        . '<td class="text-right" style="background: #efebb9;">' . number_format(${'tot' . sprintf("%02d", $n) . '_pc_area'}) . '</td>';  //AREA SUMMERY
                                            }
            ...
            <div class="row">
                <div class="col-lg-12 col-xs-12">
                    ...
                    if (!empty($DailySalesQty) && isset($DailySalesQty)) {
                        ...
                        ?>
                        <h2>Total Actual Achievement Quantity with PC <?php
                    if (!empty($categoryLineData) && isset($categoryLineData)) {
