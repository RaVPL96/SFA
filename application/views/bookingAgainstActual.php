...
<div class="content-wrapper">
    ...
    <section class="content-header">
        <h1>
            Booking Vs Actual Quantities & Values 
    ...
    <section class="content" style="overflow:hidden;">
        ...
        <div class="row">
            <div class="col-md-12">  
                <form class="form-horizontal" id="myForm" action="<?= base_url('BookingAgainstActualController/bookingAgainstActual') ?>" method="post">
                    <div class="col-md-4">                            
                    ...
                    <div class="col-md-5">
                        <div class="form-group">
                            <input type="hidden" class="form-control" id="saleType" name="saleType" value="B">
                ...
                <div class="row">
                    ...
                    <div class="col-lg-12 col-xs-12">
                        ...
                        if (!empty($DailySales) && isset($DailySales)) {
                            ...
                            // BOOKING VS ACTUAL AREA TOTA ACTUAL
                            $tot01_area_a = 0;
                            $tot02_area_a = 0;
                            $tot03_area_a = 0;
                            $tot04_area_a = 0;
                            ...
                            // BOOKING VS ACTUAL REAGION ACTUAL VALUES
                            ...
                            // BOOKING VS ACTUAL - ACTUAL VALUES
                            ...
                            for ($n = 1; $n <= date('t', strtotime($datepickermonth)); $n++) {
                                ...
                                $strDayColWithPC .= '<th class="fix" colspan="4">' . $monthName . '-' . sprintf("%02d", $n) . '</th>';
                                $strDayColValPC .= '<th>' . sprintf("%02d", $n) . ' B-Value</th><th>' . sprintf("%02d", $n) . ' A-Value</th><th style="background: #efebb9;">' . sprintf("%02d", $n) . ' B-PC</th><th style="background: #efebb9;">' . sprintf("%02d", $n) . ' A-PC</th>';
                            }
                            //echo $strDayCol;
                            $strBack = '';
                            for ($k = 1; $k <= 6; $k++) {
                            ...
                            <table id="attendance_table" class="table table-hover presentation">
                                <thead>
                                    <tr>
                                        ...
                                        <th class="fix"></th> 
    <?= $strDayColWithPC ?> 
                                        <th class="fix" colspan="11" style="text-align: center;"><?= $monthName ?>-Total</th>
    ...
    <?= $strDayColValPC ?> 
                                        <th>Target</th>
                                        <th><?= $monthName ?>-B Total</th> 
                                        <th><?= $monthName ?>-A Total</th> 
                                        ...
                                        <th style="background: #efebb9;"><?= $monthName ?>-B-Total-PC</th>
                                        <th style="background: #efebb9;"><?= $monthName ?>-A-Total-PC</th>
    <?= $strBack ?> 
                                    </tr>
                                </thead>
                                <tbody>
                                        ...
                                        foreach ($DailySales as $v) {
                                            ...
                                            for ($n = 1; $n <= date('t', strtotime($datepickermonth)); $n++) {
                                                ...
                                                ${'tot' . sprintf("%02d", $n) . '_v_a'} = ${'tot' . sprintf("%02d", $n) . '_v_a'} + $v[$monthName . '-' . $n . '-A'];
                                                ${'tot' . sprintf("%02d", $n) . '_pc'} = ${'tot' . sprintf("%02d", $n) . '_pc'} + $v[$monthName . '-' . $n . '-pc'];
                                                ${'tot' . sprintf("%02d", $n) . '_pc_a'} = ${'tot' . sprintf("%02d", $n) . '_pc_a'} + $v[$monthName . '-' . $n . '-pc-A'];

                                                if ($areaNameCurrent == $v['area_name']) {//we are in same area
                                                    //AREA TOTAL
                                                    ${'tot' . sprintf("%02d", $n) . '_area'} = ${'tot' . sprintf("%02d", $n) . '_area'} + $v[$monthName . '-' . $n];
                                                    ${'tot' . sprintf("%02d", $n) . '_area_a'} = ${'tot' . sprintf("%02d", $n) . '_area_a'} + $v[$monthName . '-' . $n . '-A'];
                                                    ${'tot' . sprintf("%02d", $n) . '_pc_area'} = ${'tot' . sprintf("%02d", $n) . '_pc_area'} + $v[$monthName . '-' . $n . '-pc'];
                                                    ${'tot' . sprintf("%02d", $n) . '_pc_area_a'} = ${'tot' . sprintf("%02d", $n) . '_pc_area_a'} + $v[$monthName . '-' . $n . '-pc-A'];
                                                }

                                                if ($region_name_current == $v['region_name']) {
                                                    $strTest .= $v['ag_name'];
                                                    ${'tot' . sprintf("%02d", $n) . '_region'} = ${'tot' . sprintf("%02d", $n) . '_region'} + $v[$monthName . '-' . $n];
                                                    ${'tot' . sprintf("%02d", $n) . '_region_a'} = ${'tot' . sprintf("%02d", $n) . '_region_a'} + $v[$monthName . '-' . $n . '-A'];
                                                    ${'tot' . sprintf("%02d", $n) . '_pc_region'} = ${'tot' . sprintf("%02d", $n) . '_pc_region'} + $v[$monthName . '-' . $n . '-pc'];
                                                    ${'tot' . sprintf("%02d", $n) . '_pc_region_a'} = ${'tot' . sprintf("%02d", $n) . '_pc_region_a'} + $v[$monthName . '-' . $n . '-pc-A'];
                                                }
                                                ...
                                                if ($v[$monthName . '-' . $n] != 0) {
                                                    $strRowWithPc .= '<td class="text-right">' . number_format($v[$monthName . '-' . $n]) . '</td>'
                                                            . '<td class="text-right">' . number_format($v[$monthName . '-' . $n . '-A']) . '</td>'
                                                            . '<td class="text-right" style="background: #efebb9;">' . number_format($v[$monthName . '-' . $n . '-pc']) . '</td>'
                                                            . '<td class="text-right" style="background: #efebb9;">' . number_format($v[$monthName . '-' . $n . '-pc-A']) . '</td>';
                                                } else {
                                            ...
                                            if ($max == 28) {
                                                ...
                                                $rowTotPc_A = $v[$monthName . '-1-pc-A'] + $v[$monthName . '-2-pc-A'] + $v[$monthName . '-3-pc-A'] + $v[$monthName . '-4-pc-A'] + $v[$monthName . '-5-pc-A'] + $v[$monthName . '-6-pc-A'] + $v[$monthName . '-7-pc-A'] + $v[$monthName . '-8-pc-A'] + $v[$monthName . '-9-pc-A'] + $v[$monthName . '-10-pc-A'] + $v[$monthName . '-11-pc-A'] + $v[$monthName . '-12-pc-A'] + $v[$monthName . '-13-pc-A'] + $v[$monthName . '-14-pc-A'] + $v[$monthName . '-15-pc-A'] + $v[$monthName . '-16-pc-A'] + $v[$monthName . '-17-pc-A'] + $v[$monthName . '-18-pc-A'] + $v[$monthName . '-19-pc-A'] + $v[$monthName . '-20-pc-A'] + $v[$monthName . '-21-pc-A'] + $v[$monthName . '-22-pc-A'] + $v[$monthName . '-23-pc-A'] + $v[$monthName . '-24-pc-A'] + $v[$monthName . '-25-pc-A'] + $v[$monthName . '-26-pc-A'] + $v[$monthName . '-27-pc-A'] + $v[$monthName . '-28-pc-A'];
                                            } elseif ($max == 29) {
                                                ...
                                                $rowTotPc_A = $v[$monthName . '-1-pc-A'] + $v[$monthName . '-2-pc-A'] + $v[$monthName . '-3-pc-A'] + $v[$monthName . '-4-pc-A'] + $v[$monthName . '-5-pc-A'] + $v[$monthName . '-6-pc-A'] + $v[$monthName . '-7-pc-A'] + $v[$monthName . '-8-pc-A'] + $v[$monthName . '-9-pc-A'] + $v[$monthName . '-10-pc-A'] + $v[$monthName . '-11-pc-A'] + $v[$monthName . '-12-pc-A'] + $v[$monthName . '-13-pc-A'] + $v[$monthName . '-14-pc-A'] + $v[$monthName . '-15-pc-A'] + $v[$monthName . '-16-pc-A'] + $v[$monthName . '-17-pc-A'] + $v[$monthName . '-18-pc-A'] + $v[$monthName . '-19-pc-A'] + $v[$monthName . '-20-pc-A'] + $v[$monthName . '-21-pc-A'] + $v[$monthName . '-22-pc-A'] + $v[$monthName . '-23-pc-A'] + $v[$monthName . '-24-pc-A'] + $v[$monthName . '-25-pc-A'] + $v[$monthName . '-26-pc-A'] + $v[$monthName . '-27-pc-A'] + $v[$monthName . '-28-pc-A'] + $v[$monthName . '-29-pc-A'];
                                            } elseif ($max == 30) {
                                                ...
                                                $rowTotPc_A = $v[$monthName . '-1-pc-A'] + $v[$monthName . '-2-pc-A'] + $v[$monthName . '-3-pc-A'] + $v[$monthName . '-4-pc-A'] + $v[$monthName . '-5-pc-A'] + $v[$monthName . '-6-pc-A'] + $v[$monthName . '-7-pc-A'] + $v[$monthName . '-8-pc-A'] + $v[$monthName . '-9-pc-A'] + $v[$monthName . '-10-pc-A'] + $v[$monthName . '-11-pc-A'] + $v[$monthName . '-12-pc-A'] + $v[$monthName . '-13-pc-A'] + $v[$monthName . '-14-pc-A'] + $v[$monthName . '-15-pc-A'] + $v[$monthName . '-16-pc-A'] + $v[$monthName . '-17-pc-A'] + $v[$monthName . '-18-pc-A'] + $v[$monthName . '-19-pc-A'] + $v[$monthName . '-20-pc-A'] + $v[$monthName . '-21-pc-A'] + $v[$monthName . '-22-pc-A'] + $v[$monthName . '-23-pc-A'] + $v[$monthName . '-24-pc-A'] + $v[$monthName . '-25-pc-A'] + $v[$monthName . '-26-pc-A'] + $v[$monthName . '-27-pc-A'] + $v[$monthName . '-28-pc-A'] + $v[$monthName . '-29-pc-A'] + $v[$monthName . '-30-pc-A'];
                                            } elseif ($max == 31) {
                                                ...
                                                $rowTotPc_A = $v[$monthName . '-1-pc-A'] + $v[$monthName . '-2-pc-A'] + $v[$monthName . '-3-pc-A'] + $v[$monthName . '-4-pc-A'] + $v[$monthName . '-5-pc-A'] + $v[$monthName . '-6-pc-A'] + $v[$monthName . '-7-pc-A'] + $v[$monthName . '-8-pc-A'] + $v[$monthName . '-9-pc-A'] + $v[$monthName . '-10-pc-A'] + $v[$monthName . '-11-pc-A'] + $v[$monthName . '-12-pc-A'] + $v[$monthName . '-13-pc-A'] + $v[$monthName . '-14-pc-A'] + $v[$monthName . '-15-pc-A'] + $v[$monthName . '-16-pc-A'] + $v[$monthName . '-17-pc-A'] + $v[$monthName . '-18-pc-A'] + $v[$monthName . '-19-pc-A'] + $v[$monthName . '-20-pc-A'] + $v[$monthName . '-21-pc-A'] + $v[$monthName . '-22-pc-A'] + $v[$monthName . '-23-pc-A'] + $v[$monthName . '-24-pc-A'] + $v[$monthName . '-25-pc-A'] + $v[$monthName . '-26-pc-A'] + $v[$monthName . '-27-pc-A'] + $v[$monthName . '-28-pc-A'] + $v[$monthName . '-29-pc-A'] + $v[$monthName . '-30-pc-A'] + $v[$monthName . '-31-pc-A'];
                                            }
                                            $avgPCperDay = 0;
                                            if ($actual_working_count == 0) {
                                                $avgPCperDay = 'NA';
                                            ...
                                            $totF = $totF + $v[$monthName . '-Total'];
                                            $totF_A = $totF_A + $v[$monthName . '-Total-A'];

                                            if ($areaNameCurrent == $v['area_name']) {//we are in same area
                                                //AREA TOTAL
                                                $AreatotF = $AreatotF + $v[$monthName . '-Total'];
                                                $AreatotF_A = $AreatotF_A + $v[$monthName . '-Total-A'];
                                            }
                                            if ($region_name_current == $v['region_name']) {
                                                $RegiontotF = $RegiontotF + $v[$monthName . '-Total'];
                                                $RegiontotF_A = $RegiontotF_A + $v[$monthName . '-Total-A'];
                                            }

                                            $totPcF = $totPcF + $rowTotPc;
                                            $totPcF_A = $totPcF_A + $rowTotPc_A;
                                            if ($areaNameCurrent == $v['area_name']) {//we are in same area
                                                //AREA TOTAL
                                                $AreatotPcF = $AreatotPcF + $rowTotPc; //AREA TOTAL
                                                $AreatotPcF_A = $AreatotPcF_A + $rowTotPc_A; //AREA TOTAL
                                            }
                                            if ($region_name_current == $v['region_name']) {
                                                $RegiontotPcF = $RegiontotPcF + $rowTotPc; //AREA TOTAL
                                                $RegiontotPcF_A = $RegiontotPcF_A + $rowTotPc_A; //AREA TOTAL
                                            }
//PRINT AREA TOTAL AT THE END
                                            if ($areaNameCurrent == $v['area_name']) {//we are in same area
                                                ...
                                                if (!empty($AreaID) && isset($AreaID) && $AreaID == '-1') {//all island report requested
                                                    for ($n = 1; $n <= date('t', strtotime($datepickermonth)); $n++) {
                                                         ...
                                                         */
                                                        $AreastrRowFootWithPC .= '<td class="text-right">' . number_format(${'tot' . sprintf("%02d", $n) . '_area'}) . '</td>'
                                                                . '<td class="text-right">' . number_format(${'tot' . sprintf("%02d", $n) . '_area_a'}) . '</td>'
                                                                . '<td class="text-right" style="background: #efebb9;">-</td>'
                                                        ...
                                                        ${'tot' . sprintf("%02d", $n) . '_area_a'} = $v[$monthName . '-' . $n . '-A'];
                                                        ${'tot' . sprintf("%02d", $n) . '_pc_area'} = $v[$monthName . '-' . $n . '-pc'];
                                                        ${'tot' . sprintf("%02d", $n) . '_pc_area_a'} = $v[$monthName . '-' . $n . '-pc-A'];
                                                    }
                                                    $PP = 'target not set';
                                                    if ($AreaTotal != 0) {
                                                    ...
                                                    . '<td class="text-right" style="font-weight:bold;">' . number_format($AreatotF_A) . '</td>'
                                                    ...
                                                    . '<td class="text-right" style="background: #efebb9;font-weight:bold;">' . number_format($AreatotPcF_A) . '</td>'
                                                    . $strBackFootArea
                                                    . '</tr>';
                                                    $areaNameCurrent = $v['area_name'];
                                                    $AreatotF = $v[$monthName . '-Total'];
                                                    $AreatotF_A = $v[$monthName . '-Total-A'];
                                                    $AreaTotal = $targetVal; //area total Target
                                                    $AreatotPcF = $rowTotPc;
                                                    $AreatotPcF_A = $rowTotPc_A;
                                                    ...
                                                    if ($region_name_current != $v['region_name']) {//to print a new region but total lines are in old region
                                                        for ($n = 1; $n <= date('t', strtotime($datepickermonth)); $n++) {
                                                             ...
                                                             */
                                                            $RegionstrRowFootWithPC .= '<td class="text-right">' . number_format(${'tot' . sprintf("%02d", $n) . '_region'}) . '</td>'
                                                                    . '<td class="text-right">' . number_format(${'tot' . sprintf("%02d", $n) . '_region_a'}) . '</td>'
                                                                    . '<td class="text-right" style="background: #efebb9;">-</td>'
                                                                    . '<td class="text-right" style="background: #efebb9;">-</td>';  //AREA SUMMERY
                                                            ///RESET VALUES OF AREA TO NEW AREA
                                                            ${'tot' . sprintf("%02d", $n) . '_region'} = 0; // $v[$monthName . '-' . $n];
                                                            ${'tot' . sprintf("%02d", $n) . '_region_a'} = 0; // $v[$monthName . '-' . $n];
                                                            ${'tot' . sprintf("%02d", $n) . '_pc_region'} = 0; // $v[$monthName . '-' . $n . '-pc'];
                                                            ${'tot' . sprintf("%02d", $n) . '_pc_region_a'} = 0; // $v[$monthName . '-' . $n . '-pc'];
                                                        }
                                                        $PP = 'target not set';
                                                        if ($RegionTotal != 0) {
                                                        ...
                                                        . '<td class="text-right" style="font-weight:bold;">' . number_format($RegiontotF_A) . '</td>'
                                                        ...
                                                        . '<td class="text-right" style="background: #efebb9;font-weight:bold;">' . number_format($RegiontotPcF_A) . '</td>'
                                                        . $strBackFootRegion
                                                        . '</tr>';
                                                        ...
                                                        $RegiontotF_A = $v[$monthName . '-Total-A'];
                                                        ...
                                                        $RegiontotPcF_A = $rowTotPc_A; //AREA TOTAL
                                                        $strBackFootRegion = '';
                                                        for ($n = 1; $n <= date('t', strtotime($datepickermonth)); $n++) {
                                                            if ($region_name_current == $v['region_name']) {
                                                                $strTest .= $v['ag_name'];
                                            ...
                                            . '<td class="text-right" style="font-weight:bold;">' . number_format($v[$monthName . '-Total-A']) . '</td>'
                                            ...
                                            . '<td class="text-right" style="background: #efebb9;font-weight:bold;">' . number_format($rowTotPc_A) . '</td>'
                                            . $strBackValues
                                            . '</tr>';
                                                ...
                                                if (!empty($AreaID) && isset($AreaID) && $AreaID == '-1' && count($DailySales) == $currentRow) {//in last row -all island report requested
                                                    for ($n = 1; $n <= date('t', strtotime($datepickermonth)); $n++) {
                                                        ...
                                                        $AreastrRowFootWithPC .= '<td class="text-right">' . number_format(${'tot' . sprintf("%02d", $n) . '_area'}) . '</td>'
                                                                . '<td class="text-right">' . number_format(${'tot' . sprintf("%02d", $n) . '_area_a'}) . '</td>'
                                                                . '<td class="text-right" style="background: #efebb9;">' . number_format(${'tot' . sprintf("%02d", $n) . '_pc_area'}) . '</td>'
                                                                . '<td class="text-right" style="background: #efebb9;">' . number_format(${'tot' . sprintf("%02d", $n) . '_pc_area_a'}) . '</td>';
                                                        ;  //AREA SUMMERY
                                                    }
                                                    ...
                                                    for ($n = 1; $n <= date('t', strtotime($datepickermonth)); $n++) {
                                                        $RegionstrRowFootWithPC .= '<td class="text-right">' . number_format(${'tot' . sprintf("%02d", $n) . '_region'}) . '</td>'
                                                                . '<td class="text-right">' . number_format(${'tot' . sprintf("%02d", $n) . '_region_a'}) . '</td>'
                                                                . '<td class="text-right" style="background: #efebb9;">' . number_format(${'tot' . sprintf("%02d", $n) . '_pc_region'}) . '</td>'
                                                                . '<td class="text-right" style="background: #efebb9;">' . number_format(${'tot' . sprintf("%02d", $n) . '_pc_region_a'}) . '</td>';  //AREA SUMMERY
                                                        ///RESET VALUES OF AREA TO NEW AREA
                                                        //${'tot' . sprintf("%02d", $n) . '_region'} = $v[$monthName . '-' . $n];
                                                        //${'tot' . sprintf("%02d", $n) . '_pc_region'} = $v[$monthName . '-' . $n . '-pc'];
                                                    }
                                        ...
                                        for ($n = 1; $n <= date('t', strtotime($datepickermonth)); $n++) {
                                            ...
                                            $strRowFootWithPC .= '<td class="text-right">' . number_format(${'tot' . sprintf("%02d", $n) . '_v' }) . '</td>'
                                                    . '<td class="text-right">' . number_format(${'tot' . sprintf("%02d", $n) . '_v_a' }) . '</td>'
                                                    . '<td class="text-right" style="background: #efebb9;">' . number_format(${'tot' . sprintf("%02d", $n) . '_pc'}) . '</td>'
                                                    . '<td class="text-right" style="background: #efebb9;">' . number_format(${'tot' . sprintf("%02d", $n) . '_pc_a'}) . '</td>';
                                        }
                                        ?>
                                </tbody>
                                <tfoot>
                                    ...
                                    . '<td class="text-right" style="font-weight:bold;">' . number_format($totF_A) . '</td>'
                                    ...
                                    . '<td class="text-right" style="background: #efebb9;font-weight:bold;">' . number_format($totPcF_A) . '</td>'
                                    . $strBackFoot
                                    . '</tr>';
                                    ?>
                                </tfoot>
...
if (!empty($DailySalesQty) && isset($DailySalesQty)) {
    ...
    for ($n = 1; $n <= date('t', strtotime($datepickermonth)); $n++) {
        $strDayCol .= '<th>' . $monthName . '-' . sprintf("%02d", $n) . '</th>';
        $strDayColWithPC .= '<th class="fix" colspan="4">' . $monthName . '-' . sprintf("%02d", $n) . '</th>';
        $strDayColValPC .= '<th>' . sprintf("%02d", $n) . ' B Qty</th><th>' . sprintf("%02d", $n) . '  A Qty</th><th style="background: #efebb9;">' . sprintf("%02d", $n) . ' B-PC</th><th style="background: #efebb9;">' . sprintf("%02d", $n) . ' A-PC</th>';
    }
    //echo $strDayCol;
    $strBack = '';
    for ($k = 1; $k <= 6; $k++) {
                                        ...
                                        foreach ($DailySalesQty as $v) {
                                            ...
                                            for ($n = 1; $n <= date('t', strtotime($datepickermonth)); $n++) {
                                                ...
                                                if ($v[$monthName . '-' . $n] != 0) {
                                                    ...
                                                    $strRowWithPc .= '<td class="text-right">-</td><td class="text-right">-</td>'
                                                            . '<td class="text-right" style="background: #efebb9;">-</td>'
                                                            . '<td class="text-right" style="background: #efebb9;">-</td>';
                                                }
                                                $strRowFoot .= '<td class="text-right">' . number_format(${'tot' . sprintf("%02d", $n) . '_v'}) . '</td>'
                                                ...
                                                if (!empty($AreaID) && isset($AreaID) && $AreaID == '-1' && count($DailySalesQty) == $currentRow) {//in last row -all island report requested
                                                    for ($n = 1; $n <= date('t', strtotime($datepickermonth)); $n++) {
                                                        ...
                                                        $AreastrRowFootWithPC .= '<td class="text-right">' . number_format(${'tot' . sprintf("%02d", $n) . '_area'}) . '</td>'
                                                                . '<td class="text-right">' . number_format(${'tot' . sprintf("%02d", $n) . '_area_a'}) . '</td>'
                                                                . '<td class="text-right" style="background: #efebb9;">' . number_format(${'tot' . sprintf("%02d", $n) . '_pc_area'}) . '</td>'
                                                                . '<td class="text-right" style="background: #efebb9;">' . number_format(${'tot' . sprintf("%02d", $n) . '_pc_area_a'}) . '</td>';  //AREA SUMMERY
                                                    }
