    ...
    <section class="content">
        <div class="row">
            <div class="col-md-6">
                ...
                <form role="form" action="<?=  base_url('index.php/users/updateAuth')?>" method="post">
                <div class="box">
                ...
                <div class="box-body">
                  <table class="table table-bordered">
                    ...
                    if (!empty($authList) && isset($authList)) {                        
                        foreach ($authList as $auth) {
                            $chked = '';
                            $class='';
                            $status='Inactive';
                            if (!empty($authGrant) && isset($authGrant)) {
                                if (in_array($auth['group_id'], $authGrant)) {
                                   $chked = 'checked';
                                   $class='bg-green';
                                   $status='Active';
                                } else {
                            ...
                            echo '<tr>';
                            echo '<td><div class="checkbox"><label><input type="checkbox" name="authCode[]" value="' . $auth['group_id'] . '" ' . $chked . '/></label></div></td>';
                            echo '<td>' . $auth['name'] . '</td>';
                            echo '<td>' . $auth['display_name'] . '</td>';
            ...
            foreach ($userlist as $user) {
                ...
                <td>' . $user['email'] . '</td>
                    <td>' . $user['name'] . '</td>
                        <td><a href="' . base_url('index.php/users/allowUserAccess/' . $user['username']) . '">Edit</a></td>
            </tr>';
            }
            ?>          
					</tbody>			
