                    ...
                    if (!empty($DailySales) && isset($DailySales)) {
                        ...
                        foreach ($DailySales as $v) {
                            ...
                            for ($k = 1; $k <= 6; $k++) {
                                ...
                                $strBackFoot .= '<td class="text-right">' . number_format(${'totF' . $k}) . '</td>';
                            }
                            ...
                            . '<td>' . $v['cd'] . '</td>'
                            . $strRow
                            . '<td class="text-right">' . number_format($v[$monthName . '-Total']) . '</td>'
                            . $strBackValues
                            . '</tr>';
                        }
                        ?>
                        ...
                        <table id="attendance_table" class="table table-hover">
                            <thead>
                                ...
                                <tr>
                                    ...
                                    <th>Target vs  Cum Sale With Dir. (%)</th> 
                                    <th>WD Variance</th> 
                            ...
                            <tbody>
                                ...
                                foreach ($DailySales as $v) {
                                    ...
                                    $avgPCperDay = $rowTotPc / $actual_working_count;
                                    ...
                                    . '<td>' . ((($v[$monthName . '-Total'] - $expectedTargetUptoDate) < 0 ? "(" . number_format(abs($v[$monthName . '-Total'] - $expectedTargetUptoDate)) . ")" : number_format($v[$monthName . '-Total'] - $expectedTargetUptoDate))) . '</td>'
                                    . '<td>' . number_format($persentage) . '%</td>'
                                    . '<td>' . ((($workingDayVarience) < 0 ? "(" . number_format(abs($workingDayVarience)) . ")" : number_format($workingDayVarience))) . '</td>'
                                    . '<td>' . number_format($avgSale) . '</td>'
                                    . '<td>' . number_format($avgPCperDay, 1) . '</td>'
