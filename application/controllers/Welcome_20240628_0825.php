...
class Welcome extends CI_Controller {
    ...
    public function index($msg = null) {
        ...
        if (!empty($data['sess']) && isset($data['sess']) && $data['sess']['logged_in']) {
            ...
            $data['FromDate'] = $FromDate = str_replace('/', '-', trim($RangeArr[0]));
            $data['ToDate'] = $ToDate = str_replace('/', '-', trim($RangeArr[1]));
            $data['targetData'] = $this->SalesreportModule->/*getTarget*/getTargetNewDemarcation(date('Y', strtotime($FromDate)), date('m', strtotime($FromDate)), $areaIDPassed);
            ...
            if (!empty($_POST['areaID']) && isset($_POST['areaID'])) {
                ...
                if ($data['channelID'] == 1) {
                    $data['SalesData'] = $this->SalesreportModule->getDailyPcTotals($area_id, $territoryID, $routeID, $rangeID, $billMethod, $FromDate, $ToDate, 'summery');
                    //$data['SalesData'] = $this->SalesreportModule->getDailyPcTotalsNew($area_id, $territoryID, $routeID, $rangeID, $billMethod, $FromDate, $ToDate, 'summery');
                } else {
...
//              echo $data['channelID']
    ...
    public function index2024($msg = null) {
        $data['msg'] = $msg;
        $data['sess'] = $sess = $this->session->userdata('User');
        $data['pagetitle'] = 'LMA Logins';
        $data['title'] = 'LMA';
...
//              echo $data['channelID']
            ...
            $this->load->view('template/header', $data);
            $this->load->view('welcome_message_2024');
            $this->load->view('template/footer');
        } else {
            header('location:' . base_url('index.php/users/index'));
        } /* echo 'post';		print_r($_POST);		echo 'get';		print_r($_GET); */
    ...
    public function sales($msg = null) {
        $data['msg'] = $msg;
        $data['sess'] = $sess = $this->session->userdata('User');
        $data['pagetitle'] = 'LMA Logins';
        $data['title'] = 'LMA';
        if (!empty($data['sess']) && isset($data['sess']) && $data['sess']['logged_in']) {
            ...
            $data['targetData'] = $this->SalesreportModule->getTargetNewDemarcation(date('Y', strtotime($FromDate)), date('m', strtotime($FromDate)), $areaIDPassed);
            ////print_r($data['targetData']);
            //    die();
            if (!empty($_POST['billMethod']) && isset($_POST['billMethod'])) {
                $data['billMethod'] = $billMethod = $_POST['billMethod'];
            ...
            if (!empty($_POST['areaID']) && isset($_POST['areaID'])) {
                ...
                if ($data['channelID'] == 1) {
                    ...
                    $data['SalesData'] = $this->SalesreportModule->getDailyPcTotalsNewNewDemarcation($area_id, $territoryID, $routeID, $rangeID, $billMethod, $FromDate, $ToDate, 'summery', $data['channelID']);
                }
//print_r($data['SalesData']);
                ...
                if ($data['channelID'] == 1) {
                    ...
                    $data['SalesDataTerritory'] = $this->SalesreportModule->getDailyPcTotalsNewNewDemarcation($area_id, $territoryID, $routeID, $rangeID, $billMethod, $FromDate, $ToDate, 'summery_territory', $data['channelID']);
                }
                ...
                if (!empty($territoryIDPassed) && isset($territoryIDPassed)) {
                    ...
                    if ($data['channelID'] == 1) {
                        ...
                        $data['SalesDataDetails'] = $this->SalesreportModule->getDailyPcTotalsNewNewDemarcation($area_id, $territoryID, $routeID, $rangeID, $billMethod, $FromDate, $ToDate, 'details', $data['channelID']);
                    }
                }
            }
            //echo $billMethod;
            ...
            if ($sess['grade_id'] != 5 && $sess['grade_id'] != 4) {//not for sales rep and other users also except ASE
                ...
                foreach ($data['ChannelMapping'] as $c) {
                    ...
                    foreach ($data['RangeList'] as $r) {
                        //echo "SalesData_Area_Summery_".$c['channel_id']."_".$r['range_id'];
                        if ($data['channelID'] == 1) {
                            $data['SalesData_Area_Summery_' . $c['channel_id'] . '_' . $r['range_id']] = $this->SalesreportModule->getDailyPcTotalsNewDemarcation($area_id, $territoryID, $routeID, $r['range_id'], $billMethod, $FromDate, $ToDate, 'summery_area', $c['channel_id']);
                        } else {
                            $data['SalesData_Area_Summery_' . $c['channel_id'] . '_' . $r['range_id']] = $this->SalesreportModule->getDailyPcTotalsNewNewDemarcation($area_id, $territoryID, $routeID, $r['range_id'], $billMethod, $FromDate, $ToDate, 'summery_area', $c['channel_id']);
                        }
                    }
                }
                /*
                  $rangeID=1;//Area Summery
                  $data['SalesData_Area_Summery'] = $this->SalesreportModule->getDailyPcTotalsNewDemarcation($area_id, $territoryID, $routeID, $rangeID, $billMethod, $FromDate, $ToDate, 'summery_area');
                  $rangeID=2;//Area Summery
                  $data['SalesData_Area_Summery_2'] = $this->SalesreportModule->getDailyPcTotalsNewDemarcation($area_id, $territoryID, $routeID, $rangeID, $billMethod, $FromDate, $ToDate, 'summery_area');
                 */
            }
    ...
    function home($msg = null) {
        ...
        if (!empty($data['sess']) && isset($data['sess']) && $data['sess']['logged_in']) {
            ...
            if (!empty($_POST['datepickermonth']) && isset($_POST['datepickermonth'])) {
            ...
            //$data['DailySales'] = $this->SalesreportModule->getDailySales($data['AreaID'], $data['RangeID']);
            if (!empty($data['RangeID']) && isset($data['RangeID']) && $data['RangeID'] != null && $data['RangeID'] != -1) {
                $FromDate = date('Y-m-01', strtotime($data['datepickermonth']));
                if ($data['datepickermonth'] === date('Y-m')) {//this is current month
                    $ToDate = date('Y-m-d');
                } else {
                    $ToDate = date('Y-m-t', strtotime($data['datepickermonth']));
                }
                ...
                if ($data['RangeID'] == 'D') {
                    if (!empty($_POST['category']) && isset($_POST['category'])) {//category sales requested
                        ...
                        $data['getTotalSalesandPc'] = $this->SalesreportModule->getTotalSalesandPcCategory($data['AreaID'], $data['RangeID'], $data['datepickermonth'], $data['categoryID']);
                        $data['getTotalSalesandPcFull'] = $this->SalesreportModule->getTotalSalesandPc($data['AreaID'], $data['RangeID'], $data['datepickermonth']);
                        $data['DailySales'] = $this->SalesreportModule->getDailySalesCategory($data['AreaID'], $data['RangeID'], $data['datepickermonth'], $data['categoryID']);
                        $data['DailySalesQty'] = $this->SalesreportModule->getDailySalesCategory($data['AreaID'], $data['RangeID'], $data['datepickermonth'], $data['categoryID'], 'quantity');
                    } else {//total sale requested
                        $data['targetData'] = $this->SalesreportModule->getTarget(date('Y', strtotime($data['datepickermonth'])), date('m', strtotime($data['datepickermonth'])), $data['AreaID']);
                        $data['getTotalSalesandPc'] = $this->SalesreportModule->getTotalSalesandPc($data['AreaID'], $data['RangeID'], $data['datepickermonth']);
                        $data['DailySales'] = $this->SalesreportModule->getDailySales($data['AreaID'], $data['RangeID'], $data['datepickermonth']);
                    }
                    //$data['DailySalesD'] = $this->SalesreportModule->getDItemSales($data['AreaID']);
                } else {
                    if (!empty($_POST['category']) && isset($_POST['category'])) {//category sales requested
                        $data['categoryID'] = $_POST['category'];
                        $data['categoryLineData'] = $this->SalesreportModule->getCategoryList(3,$data['categoryID']);
                        ...
                        $data['getTotalSalesandPc'] = $this->SalesreportModule->getTotalSalesandPc($data['AreaID'], $data['RangeID'], $data['datepickermonth']);
                        $data['DailySales'] = $this->SalesreportModule->getDailySales($data['AreaID'], $data['RangeID'], $data['datepickermonth']);
                    }//$data['DailySalesNenaposha'] = $this->SalesreportModule->getNenaposhaItemSales($data['AreaID']);
                    //$data['DailySalesDB'] = $this->SalesreportModule->getDbItemSales($data['AreaID']);
                    //$data['DailySalesSoya'] = $this->SalesreportModule->getSoyaItemSales($data['AreaID']);
                }
            } else {
    ...
    //NEW DEMARCATION REPORT START 2023 10 22
            ...
            if (!empty($data['RangeID']) && isset($data['RangeID']) && $data['RangeID'] != null && $data['RangeID'] != -1) {
                ...
                if ($data['RangeID'] == 'D') {
                    if (!empty($_POST['category']) && isset($_POST['category'])) {//category sales requested
                        $data['categoryID'] = $_POST['category'];
                        $data['categoryLineData'] = $this->SalesreportModule->getCategoryList(3,$data['categoryID']);
                        $data['getTotalSalesandPc'] = $this->SalesreportModule->getTotalSalesandPcCategoryNewDemarcation($data['AreaID'], $data['RangeID'], $data['datepickermonth'], $data['categoryID']);
                        $data['getTotalSalesandPcFull'] = $this->SalesreportModule->getTotalSalesandPcNewDemarcation($data['AreaID'], $data['RangeID'], $data['datepickermonth']);
                        $data['DailySales'] = $this->SalesreportModule->getDailySalesCategoryNewDemarcation($data['AreaID'], $data['RangeID'], $data['datepickermonth'], $data['categoryID']);
                        $data['DailySalesQty'] = $this->SalesreportModule->getDailySalesCategoryNewDemarcation($data['AreaID'], $data['RangeID'], $data['datepickermonth'], $data['categoryID'], 'quantity');
                    } else {//total sale requested
                        $checkVariable = null;
                        if(!empty($_POST['variable1']) && isset($_POST['variable1'])){
                           $checkVariable = $_POST['variable1'];

                        $data['targetData'] = $this->SalesreportModule->getTargetNewDemarcation1(date('Y', strtotime($data['datepickermonth'])), date('m', strtotime($data['datepickermonth'])), $data['AreaID'],$checkVariable);
                        }else{
                        $data['targetData'] = $this->SalesreportModule->getTargetNewDemarcation(date('Y', strtotime($data['datepickermonth'])), date('m', strtotime($data['datepickermonth'])), $data['AreaID']);

                        }
                        $data['getTotalSalesandPc'] = $this->SalesreportModule->getTotalSalesandPcNewDemarcation($data['AreaID'], $data['RangeID'], $data['datepickermonth']);
                        $data['DailySales'] = $this->SalesreportModule->getDailySalesNewDemarcation($data['AreaID'], $data['RangeID'], $data['datepickermonth']);
                    }
                    //$data['DailySalesD'] = $this->SalesreportModule->getDItemSales($data['AreaID']);
                } else {
                    if (!empty($_POST['category']) && isset($_POST['category'])) {//category sales requested
    ...
    //NEW DEMARCATION PURPOSE REPORTS
    function figures($msg = null) {
        $data['msg'] = $msg;
        $data['sess'] = $sess = $this->session->userdata('User');
        $data['pagetitle'] = 'LMA Logins';
        $data['title'] = 'LMA';
