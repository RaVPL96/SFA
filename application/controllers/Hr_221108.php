...
class Hr extends CI_Controller {
    ...
    function calActualWorking() {
        $FromDate = date('Y-m-d', strtotime("-35 days"));
        $ToDate = date('Y-m-d');
        $this->HrModule->calActualWorkings($FromDate, $ToDate);
    }

    //cal allowed promo schea and slab
    function calAllowedSchemaSlab($territory_id,$range_id) {
        echo $this->HrModule->calSchemaSlab($territory_id,$range_id);
    }

    function calPromoTarget($scheme_id,$range_id) {
        echo $this->HrModule->calPromoTarget($scheme_id,$range_id);
    }
