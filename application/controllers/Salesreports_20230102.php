...
class Salesreports extends CI_Controller {
    ...
    function cancellation($msg = null) {
        ...
        if (isset($_POST['DateRange']) && !empty($_POST['DateRange'])) {
            ...
            if (isset($_POST['sbTerritory']) && !empty($_POST['sbTerritory'])) {
                $data['sbTerritory'] = $_POST['sbTerritory'];
            }

            if (isset($_POST['sbRange']) && !empty($_POST['sbRange'])) {
                $data['sbRange'] = $_POST['sbRange'];
            }

            $data['DateRange'] = $DateRange = $_POST['DateRange'];
            $RangeArr = explode('-', $DateRange);
            echo $FromDate = str_replace('/', '-', trim($RangeArr[0]));
            //die();
            $ToDate = str_replace('/', '-', trim($RangeArr[1]));
            $data['dataInfo'] = $this->SalesreportModule->getCancellation($FromDate, $ToDate, 110);
        }
