...
class Salesoetransaction extends CI_Controller {
    ...
    function stockInvoiceView($msg = null) {
        $functionID = 67;
        $this->UsersModel->authenticateMe($functionID);
        $functionSubID = 19;
        $this->UsersModel->authenticateMeSubFunction($functionID, $functionSubID);
        $data['sess'] = $sess = $this->session->userdata('User');
        $data['title'] = 'Reports Agency Stock';
        $data['pagetitle'] = 'View List';
        $data['msg'] = $msg;
        $data['invData'] = null;
        if (!empty($_POST['idinvoice']) && isset($_POST['idinvoice'])) {
            $invNum = $_POST['idinvoice'];
            $FromDate = '';
            $ToDate = '';
            $data['invDataMain'] = $this->ItemsModel->getMaindetail($invNum);
            $data['invDataH'] = $this->ItemsModel->getFulldetail($invNum);

            
        }


        $this->load->view('secondary/oe/stock_inv_preview', $data);
    }
    ...
    function SaveOrderentry($id = null, $msg = null){
        $functionID = 46;
        $this->UsersModel->authenticateMe($functionID);
        $functionSubID = 13;
        $this->UsersModel->authenticateMeSubFunction($functionID, $functionSubID);
        ...
        $code = $_POST['estimatepartcode'];
        $upRowCount = count($code);

        $up_itemCode  = $_POST['estimatepartcode'];
        $up_itemName  = $_POST['estimatepartname'];
        $up_sellable  = $_POST['estimatepartqty'];
        $up_damageQty = $_POST['estimatedamageqty'];
        $up_unitPrice = $_POST['estimatepartprice'];
        $up_subTotal  = $_POST['estimatepartsubtotal'];
        $up_unitOfMess  = $_POST['estimatepartpriceUnit'];
        // print_r($_POST['estimatepartqty']);
        // die();
        // for($i=1;$i<$upRowCount;$i++){

        //     $up_itemName[$i]  = $_POST['estimatepartname'.$i];
        //     $up_itemCode[$i]  = $_POST['estimatepartcode'.$i];
        //     $up_sellable[$i]  = $_POST['estimatepartqty'.$i];
        //     $up_damageQty[$i] = $_POST['estimatedamageqty'.$i];
        //     $up_unitPrice[$i] = $_POST['estimatepartprice'.$i];
        //     $up_subTotal[$i]  = $_POST['estimatepartsubtotal'.$i];
        // }
        ...
        for($i=1;$i<$rowCount;$i++){
            ...
            $itemName[$i]  = $_POST['itemDes'.$i];
            $itemCode[$i]  = $_POST['itemCode'.$i];
            $sellable[$i]  = $_POST['sellableQty'.$i];
            $damageQty[$i] = $_POST['damageQty'.$i];
            $unitPrice[$i] = $_POST['unitPrice'.$i];
            $subTotal[$i]  = $_POST['subTotal'.$i];
            $itemUom[$i]  = $_POST['itemUom'.$i];
        }

            $rangExplode = explode("~",$_POST['rangeId']);
            $rangeId      = $rangExplode[0];
            $rangeName    = $rangExplode[1];
            $areaID       = $_POST['areaID'];
            $territoryID  = $_POST['territoryID'];
            $requestDate  = $_POST['requestDate'];

            $netTotal = $_POST['estimatenet'];
            $payments = $_POST['estimatePayment'];
            $balance  = $_POST['estimateBalance'];
            $comments = $_POST['estassestment'];

        //     if (!empty($_POST['requestDate']) && isset($_POST['requestDate'])) {

            
        //     $requestDate = str_replace('/', '', trim($_POST['requestDate']));
        // }

        $result = $this->ItemsModel->saveStockDetail($rowCount,$sellable,$damageQty,$unitPrice,$subTotal,$netTotal,$payments,$balance,$upRowCount,$up_itemCode,$up_itemName,$up_sellable,$up_damageQty,$up_unitPrice,$up_subTotal,$rangeId,$areaID,$territoryID,$requestDate,$itemCode,$itemName,$comments,$rangeName,$up_unitOfMess,$itemUom);

 

 
        if ($result == 1) {//Inerted 
            header('Location:' . base_url('salesoetransaction/stockEntry/-1/ok'));
        } else {
            header('Location:' . base_url('salesoetransaction/stockEntry/-1/fail'));
        }
    }


    function stockViewList($id = null, $msg = null) {
        $functionID = 46;
        $this->UsersModel->authenticateMe($functionID);
        $functionSubID = 13;
        $this->UsersModel->authenticateMeSubFunction($functionID, $functionSubID);
        ...
        $data['territoryID'] = $territoryID = $_POST['territoryID'];
        $data['requestDate'] = $requestDate = $_POST['requestDate'];

            $data['CategoryList'] = $this->SalesreportModule->getCategoryList(3);
            $rangeExplode = explode("~", $rangeId);
            
            $data['itemPriceList'] = $this->ItemsModel->getMstItems($rangeExplode[1]);
            $data['dataSet'] = 30;
        }
        ...
        $this->load->view('template/header', $data);
        $this->load->view('secondary/oe/stockViewList');
        $this->load->view('template/footer');

    }
    
    function getViewList($id = null, $msg = null) {
        ...
        if(!empty($_POST['rangeID']) && isset($_POST['rangeID']) && !empty($_POST['areaID']) && isset($_POST['areaID']) && !empty($_POST['territoryID']) && isset($_POST['territoryID'])){
            ...
            $rangeExplode = explode("~",$_POST['rangeID']);
            $rangeId = $rangeExplode[0];    
            $rangeName = $rangeExplode[1];    
            $data['areaID']      = $areaID      = $_POST['areaID'];     
            $data['territoryID'] = $territoryID = $_POST['territoryID'];


        $dateExplode = explode("-",$_POST['requestDate']);
        $fromDate = $dateExplode[0];
        $toDate   = $dateExplode[1];

        $data['dataList'] = $this->ItemsModel->getStockDetail($rangeId,$rangeName,$areaID,$territoryID,$fromDate,$toDate);
