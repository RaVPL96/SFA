...
class Itemreports extends CI_Controller {
    ...
    function getCategoryReport($msg = null) {
        $functionID = 78;
        $this->UsersModel->authenticateMe($functionID);
        $functionSubID = 64;
        $data['sess'] = $sess = $this->session->userdata('User');
        ...
        $this->load->view('template/header', $data);
        $this->load->view('secondary/reports/category/itemwisecatreport');
        $this->load->view('template/footer');
    }

    function bookingVsActualQty($msg = null) {
        $functionID = 78;
        $this->UsersModel->authenticateMe($functionID);
        $functionSubID = 64;
        $data['sess'] = $sess = $this->session->userdata('User');
        ...
        if (!empty($_POST['DateRange']) && isset($_POST['DateRange'])) {
            ...
            $data['DataSet'] = $this->ItemreportsModule->getBookingVsActual($data['AreaID'], $data['RangeID'], $FromDate, $ToDate);
            
        }
        $this->load->view('template/header', $data);
        $this->load->view('secondary/reports/category/bookingVsActual');
        $this->load->view('template/footer');
    }

    

    function dailyItemWiseSale($msg = null) {
        $functionID = 78;
        $this->UsersModel->authenticateMe($functionID);
        $functionSubID = 64;
        $data['sess'] = $sess = $this->session->userdata('User');
        ...
        if (!empty($_POST['rangeID']) && isset($_POST['rangeID'])) {
            $data['RangeID'] = $range = $_POST['rangeID'];
            $data['RangeName'] = $data['RangeID']; // $this->MasterModule->getRange($data['RangeID'] );
        }
        ...
        if (!empty($_POST['DateRange']) && isset($_POST['DateRange'])) {
            ...
            $data['DataSet'] = $this->ItemreportsModule->getDailyItemWiseSale($data['RangeID'], $FromDate, $ToDate);
//            print_r($DataSet);
        }
        $this->load->view('template/header', $data);
        $this->load->view('secondary/reports/category/dailyItemWiseSale');
        $this->load->view('template/footer');
    }
