...
class Salesreports extends CI_Controller {
    ...
    function areaSummeryItem($msg = null,$areaIDPassed=null,$rangeIDPassed=null,$territoryIDPassed=null,$dateRangePassed=null,$billingMethodPass=null,$dateBilled=null) {
        $functionID = 73;
        $this->UsersModel->authenticateMe($functionID);
        $functionSubID = 28;
        $this->UsersModel->authenticateMeSubFunction($functionID, $functionSubID);
        ...
        if (!empty($_POST['areaID']) && isset($_POST['areaID'])) {
            ...
            $data['CategoryID']=$area_id = $_POST['categoryID'];
            ...
            $data['SalesData'] = $this->SalesreportModule->getItemPcTotals($area_id, $territoryID, $categoryID, $routeID, $rangeID, $billMethod, $FromDate, $ToDate, 'summery');
            $data['SalesDataTerritory'] = $this->SalesreportModule->getItemPcTotals($area_id, $territoryID, $categoryID, $routeID, $rangeID, $billMethod, $FromDate, $ToDate, 'summery_territory');
            if(!empty($territoryIDPassed) && isset($territoryIDPassed)){
                //$territoryID=$territoryIDPassed;
                if(!empty($dateBilled) && isset($dateBilled)){
                    $FromDate=$dateBilled;
                    $ToDate=$dateBilled;
                    $data['SalesDataDetails'] = $this->SalesreportModule->getItemPcTotals($area_id, $territoryID, $categoryID, $routeID, $rangeID, $billMethod, $FromDate, $ToDate, 'details');
                }
                
            }
        }
        $this->load->view('template/header', $data);
        $this->load->view('secondary/reports/area/area_achievment_summery_item');
        $this->load->view('template/footer');
    }
    /*-----------------------------------------------------Sandun-----------------------------------------------------*/
    /*--------------------------------------Area Summery Item Wise--------------------------------------*/
