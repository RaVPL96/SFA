...
class Salesreports extends CI_Controller {
    ...
    function areaSummeryItem($msg=null, $areaIDPassed=null, $rangeIDPassed=null, $territoryIDPassed=null, $dateRangePassed=null) {
        $functionID = 73;
        $this->UsersModel->authenticateMe($functionID);
        $functionSubID = 30;
        $this->UsersModel->authenticateMeSubFunction($functionID, $functionSubID);
        ...
        if(!empty($dateRangePassed) && isset($dateRangePassed)){
            $dateRangePassed=str_replace('~', '/', trim($dateRangePassed));
            $_POST['DateRange']=$dateRangePassed;
        }
        if (!empty($_POST['areaID']) && isset($_POST['areaID'])) {
            $routeID = null;
            ...
            $data['SalesData'] = $this->SalesreportModule->getAreaSumItemWise($area_id, $categoryID, $billMethod, $FromDate, $ToDate, 'summery');
            $data['SalesDataTerritory'] = $this->SalesreportModule->getAreaSumItemWise($area_id, $categoryID, $billMethod, $FromDate, $ToDate, 'summery_territory');
            if(!empty($territoryIDPassed) && isset($territoryIDPassed)){
                $territoryIDPassed=$territoryID;
                $data['SalesDataDetails'] = $this->SalesreportModule->getAreaSumItemWise($area_id, $categoryID, $billMethod, $FromDate, $ToDate, 'details');
            }
        }
        $this->load->view('template/header', $data);
