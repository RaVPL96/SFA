...
class BookingSale extends CI_Controller
{
    /**
    ...
    function __construct()
    {
        ...
        $this->load->model("SaleBookingModule");
    ...
    function homeReport($msg = null)
    {
        ...
        if (
            ...
            $data["CategoryList"] = $this->SaleBookingModule->getCategoryList(
                3
            );
            //$data['DailySales'] = $this->SaleBookingModule->getDailySales($data['AreaID'], $data['RangeID']);
            if (
                !empty($data["RangeID"]) &&
                isset($data["RangeID"]) &&
                $data["RangeID"] != null &&
                ...
                $data[
                    "WorkingDates"
                ] = $this->SaleBookingModule->getCompanyWorkingDays(
                    $FromDate,
                    $ToDate
                );
                $data[
                ...
                if ($data["RangeID"] == "D") {
                    if (
                        ...
                        $data[
                            "categoryLineData"
                        ] = $this->SaleBookingModule->getCategoryList(
                            3,
                            $data["categoryID"]
                        );
                        $data[
                            "getTotalSalesandPc"
                        ] = $this->SaleBookingModule->getTotalSalesandPcCategoryNewDemarcation(
                            $data["AreaID"],
                            $data["RangeID"],
                        ...
                        ] = $this->SaleBookingModule->getTotalSalesandPcNewDemarcation(
                            $data["AreaID"],
                            $data["RangeID"],
                            $data["datepickermonth"],
                            $data["saleType"]
                        ...
                        $data[
                            "DailySales"
                        ] = $this->SaleBookingModule->getDailySalesCategoryNewDemarcation(
                            $data["AreaID"],
                            $data["RangeID"],
                            $data["datepickermonth"],
                            $data["categoryID"],
                        ...
                        $data[
                            "targetData"
                        ] = $this->SaleBookingModule->getTargetNewDemarcation(
                        ...
                        $data[
                            "DailySales"
                        ] = $this->SaleBookingModule->getDailySalesNewDemarcation(
                            $data["AreaID"],
                            $data["RangeID"],
                            $data["datepickermonth"],
                            $data["saleType"]
                    ...
                    //$data['DailySalesD'] = $this->SaleBookingModule->getDItemSales($data['AreaID']);
                } else {
                    // C Range
                    if (
                        !empty($_POST["category"]) &&
                        ...
                        // $data['targetData'] = $this->SaleBookingModule->getTargetNewDemarcation(date('Y', strtotime($data['datepickermonth'])), date('m', strtotime($data['datepickermonth'])), $data['AreaID']);
                        $data[
                            "getTotalSalesandPc"
                        ] = $this->SaleBookingModule->getTotalSalesandPcNewDemarcation(
                            $data["AreaID"],
                    ...
                    } //$data['DailySalesNenaposha'] = $this->SaleBookingModule->getNenaposhaItemSales($data['AreaID']);
                    //$data['DailySalesDB'] = $this->SaleBookingModule->getDbItemSales($data['AreaID']);
                    //$data['DailySalesSoya'] = $this->SaleBookingModule->getSoyaItemSales($data['AreaID']);
                }
            } else {
                /* $data['DailySalesD'] = $this->SaleBookingModule->getDItemSales($data['AreaID']);
